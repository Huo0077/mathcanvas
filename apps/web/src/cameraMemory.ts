...
let remembered: RememberedCamera | null = null
...
export function rememberCamera(documentId: string, state: CameraState): void {
  remembered = { documentId, state: copyState(state) }
}

/** 读回某个文档的相机状态；没记过、或记的是别的文档时返回 `null`（调用方回默认视角并取景）。 */
export function loadRememberedCamera(documentId: string): CameraState | null {
  if (!remembered || remembered.documentId !== documentId) return null
  return copyState(remembered.state)
}
