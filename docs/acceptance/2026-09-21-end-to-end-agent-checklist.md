...
## 已通过（2026-09-21 实跑，真模型 DeepSeek）

用 CDP 驱动**真的桌面窗口**跑完整条链，结果如下：

```
起点对象数 = 0
真模型给出草稿（含 3 条假设：底面落在 z=0 地面上 / 一个顶点在原点、三条棱沿坐标轴正方向 / 棱长 3）
已提交
确认后对象数 = 1 · 工作区 立体几何        ← 画布上真的有立方体
撤销后对象数 = 0                          ← Ctrl+Z 一步撤销
状态栏：对象 1 · 工作区 geometry3d · 0 个页面错误
```

这一遍同时验掉了两处真实缺陷（都已修，且**先有失败用例**）：

1. **一次运行在飞的时候用面板会把它打崩** —— `Cannot read properties of null (reading 'coordinator')`。
   `runtime` 是模块级变量，`confirm` / `discard` / `stop` 都会把它置空，而它们完全可能在这一轮还在
   等模型的时候被点到（上一轮留下的草稿面板还挂在界面上时尤其容易）。现在**每轮运行自己持有它的运行时**。
2. **Agent 的提交被静默丢掉** —— `store.commitCandidate` 的守卫拿 `revision` + `metadata.id` 当
   "内容没变"的判据，而候选来自 `commitTransaction`，**它不推进 `revision`**（推进它的是 store 的
   `apply`/`applyBatch`）。表现正是"面板说已提交、画布上什么都没有"，而自动保存把那份没变的空文档
   写回了本地草稿。现在判据是**内容指纹**，并在候选没带新 revision 时由 store 推进它。

## 第二遍：**非空画布**上再走一轮（2026-09-21，用户截图触发）

上面那一遍全绿，恰恰因为它从**空画布**出发。用户随后自己走了一遍（先是"建一个棱长 3 的立方体"提交成功，
再让模型处理一条直四棱柱的题），得到 `compile_failed: duplicate object id` —— 这暴露出**第三处真实缺陷**：

3. **画布上已有同类对象时，Agent 新建必然失败** —— `createIdAllocator()` 只按 kind 数数、从 1 开始，
   不知道文档里已经有 `solid-1`，于是新动作又被发成 `solid-1` → 判重拒绝。手工路径（`nextPrimitiveId`）
   一直是对的，漂移的只有动作层。修法与验证见进度文档「分配器占用集」那一节。

**这一遍的复跑步骤**（也是新的 e2e 回归用例在做的事）：先在画布上有一个对象（手工建，或先让 Agent 提交一个），
**再**发一条要新建同类对象的请求 —— 草稿必须照样出现在确认面板里，确认后对象数 +1，而不是 `没有完成`。

修完第 3 条之后，**同一个动作立刻暴露出第 4 条**（此前被它挡着，看不见）：

4. **恢复过的文档根本哈希不出来** —— 应用启动时会 `migrateLegacySolids` 把实体的子对象物化出来，
   那些子对象带着 `style: undefined` / `label: undefined`；而 Agent 的预览哈希（`canonicalContentHash`）
   当时把 `undefined` 当垃圾抛出去，于是"画布上有一个立体"就等于"运行必然失败"，错误信息只有一句
   `canonicalContentHash: unsupported value of type undefined`。现在哈希与 JSON 语义对齐（`undefined`
   等于"没有这个字段"），迁移里的"去掉标签"也从 `label: undefined` 改成**删掉这个键**。

**这两条一起的实测结论**（2026-09-21，重建 exe 后重启真窗口、真 DeepSeek）：

```
画布上已有 1 个立方体（启动恢复时迁移成 28 个图元）
「再建一个棱长 2 的立方体」 → 等待你确认 · 会新增 1 个对象（共 29 个） → 已提交 → 对象 29
「已知直四棱柱 ABCD-A1B1C1D1 … E、M、N 分别是 BC、BB1、A1D 的中点」→ 等待你确认 · 共 30 个 · 3 条假设 · 0 个页面错误
```

最后那条**刻意停在确认面板**（留给用户过目）—— 它证明的是"草稿能成型"，而这正是它此前做不到的事。

下面是**首次**要走的人工清单（留给别的机器、别的 provider，或没有 CDP 驱动时用）。
...
## 云端 provider 在本机拿不到真实回应（2026-09-21 实测）——**已定论：配置写错了**

> **⚠️ 更正（本轮最终结论，这一节下面那句"被 TLS 中间人接走"是错的）**
>
> 真实原因是**配置写错了**：那一栏的 `baseUrl` 是 `https://platform.deepseek.com`
>（DeepSeek 的**网页控制台**域名，**不是 API 地址**），模型名是 `deepseekv4.1`（官方 API 没有这个名字）。
> `platform.deepseek.com` 解析到 `60.204.2.4/.5` —— 所以那个 IP **就是用户自己配的主机**，没有人冒充；
> 而那张"自签名证书"是我用 `curl --resolve api.deepseek.com:443:60.204.2.4` **强指了一个不匹配的 SNI**
> 探出来的默认证书，属于**我自己实验的 artifact**。
>
> **教训（单独记）**：我一路都在测 `api.deepseek.com`，而被测对象配的是 `platform.deepseek.com` ——
> **验证的端点必须与被测对象实际配置的端点一致**。否则所有"对照实验"看起来都很干净，却与故障无关。
> 那条 `[peer …]` 诊断的价值正在于：它是唯一**直接读出**"应用连到哪儿"的事实，而不是我推断的。
>
> 改成 `https://api.deepseek.com/v1` + `deepseek-chat` 之后，能力验证立刻给出真实结论：
> **严格 JSON = verified、流式 = verified**、工具调用 = failed（DeepSeek 的实际行为）、图像 = failed（400）。
>
> 下面保留最初的（错误）分析作为记录：
