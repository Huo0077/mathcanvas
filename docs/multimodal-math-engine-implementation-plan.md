...
> **2026-09-18 追加**：平面几何**曲线切线 + 动点扩展**（抛物线 / 双曲线 / 圆 / 椭圆的切线，切点可沿曲线拖动或跟随动点；以动点为圆心的可调圆）、
> **轨道圆改为独立对象**（点在圆上、圆可缩放旋转），以及两条用户实测缺陷的修复（**动点与定点能建出空间直线**、**拖动动点时画面实时跟手**）。
> 最新一次全量门禁：4 个 workspace 类型检查通过、单测 **124 文件 / 1480 用例**、ESLint **0 error / 14 warning**、
> Web 生产构建通过、Playwright **111/111** 通过（`e2e/global-setup.mjs` 会按当前工作区重新构建后再预览）。
> 测试运行器已由 `vitest` `^3.2.4` 升到 **`^4.1.11`**：3.x 打进来的 birpc 有写死的 60 秒 RPC 超时，会在工作进程被同步代码卡住时**假阳性**报 `Timeout calling "onTaskUpdate"`（用例 0 失败却退码 1），上游修复见 `vitest-dev/vitest#8297`。
...
**状态：** 已完成并超出原计划。除相机 / 空间拾取 / 点线面多面体旋转体 / 隐藏线与透明面 / 法向量 / 截面剖切 / 折叠展开 / 二面角外，还有：动点绑定宿主（线 / 线段 / 射线 / 棱 / **空间圆轨道** / 面 / 曲面 / **实体内**）、**约束轨道**（`circle3` 自带圆心 + 半径 + 法向的独立对象：建完即脱钩、可平移、画布上有半径手柄直接缩放、世界轴三色环旋转，动点绑上去只沿圆周滑动且**拖动期间画面实时跟手**，轨道上的动点也能与定点连线继续构造）、**拖动旋转**（世界轴三色环 + 15° 吸附 + 属性栏角度，一次拖动一步撤销）、**测量数字常驻画布**、交面·交线·交点三个独立图元（自动枚举 + 点击创建 + 随来源重算）、自动取景与相机跨工作区记忆、**背景网格一格恒为 1 个世界单位**、渲染管道的按签名增量同步、UI 与平面几何共用同一套纸令牌。
