...
**文档版本：** v1.1（2026-09-17 状态回填：各阶段落地情况与当前门禁）  
**编制日期：** 2026-09-12  
**目标形态：** Web 优先，桌面端可通过 Tauri 封装  
**预计周期：** 18～22 周（小型跨职能团队）

> **当前状态（2026-09-17）**：本计划除 **P4（Agent 与 Provider Router）** 与 **P5（题图解析）** 外均已落地；
> P4 / P5 由用户明确排除（右侧面板精简时收敛了多模态入口，Desktop/Tauri 打包与协同同样未做）。
> 最新一次全量门禁：4 个 workspace 类型检查通过、单测 **107 文件 / 1194 用例**、ESLint **0 error / 14 warning**、
> Web 生产构建通过、Playwright **84/84** 通过（`e2e/global-setup.mjs` 会按当前工作区重新构建后再预览）。
...
| P6 立体几何 | 4 周 | Three.js、剖切、隐藏线、折叠 | 3D 工作区 Beta | **已完成**（+ 绑定宿主 / 实体内约束 / 交面交线交点 / 固定 1 单位网格） |
...
**现状（2026-09-17）**：单元 / 集成 / 浏览器三层都在跑。`npm.cmd test` 覆盖 4 个 workspace 共 **107 个文件 / 1194 个用例**；`npx playwright test` 覆盖 **84 个浏览器用例**（含 Ribbon、CAD 工作台、3D 交集预览、平面网格固定、动点拖动性能）；ESLint 0 error / 14 warning；生产构建通过。**未做**：Provider 错误分类与"Agent 指令 → Patch → 预览 → 提交"、"题图 → 解析 → DSL → 画布"这三类测试随 P4 / P5 一并排除；2D 5000 图元与 3D 100000 三角面的性能指标没有专门的基准测试（只有布尔交集与拖动增量的实测记录）。
