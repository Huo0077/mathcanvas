...
> **状态（2026-09-17）**：**三片全部已交付**（圆自带圆心 + 全部调用点 + 旧文档迁移；画布半径手柄缩放；文档收尾）。每片的实测数字都回填在切片标题下，最终门禁：单测 **121 文件 / 1430 用例**、lint **0 error / 14 warning**、Playwright **108/108**。
...
- [x] `docs/project-progress.md`：本轮条目补齐三片的 RED/证据/门禁数字与如实边界（悬空引用丢一条轨道、半径下限 0.01、不做吸附、`face3` 不在本轮），并补上"最终交付"对照表。
- [x] `docs/feature-catalog.md`：把「约束轨道」条目改写为"圆自带圆心 + 点沿圆滑动 + 画布缩放/旋转"；顺手更正旧表述（"圆心跟着那个空间点走"）。
- [x] `README.md`：三维几何一节里圆轨道那条改写 + 新增"轨道圆能在画布上缩放与旋转"一条，验证基线数字更新（1430 用例 / 108 个浏览器用例）。
- [x] 本 spec 与本计划的状态行改成"已交付"。
- [x] **补上最后一块验证缺口**：轨道圆的旋转此前只有单测，新增 e2e（拖 X 环把轨道摆斜：环仍 3 个、法向 +Z→−Y、圆心不动、一次撤销回水平）。
