...
1. **预览分两级**：**悬停**任意可交对象对 → 只给细提示（低不透明度虚线 + 一句状态文案，不给标签、不抢拾取）；**选中两个对象** → 完整预览（强调色虚线 + 分类与段数标签 + 可点击创建）。两级共用同一份内核结果，只是渲染与文案不同，因此不需要两套几何。
2. **两个实体相交只画面交线**（`intersectFaceSets` 得到的真实公共交线）；不额外提供"默认剖切平面"作为第二个候选。单实体时仍然使用既有的默认剖切平面截面，语义不变。
3. **新增 `intersectionLine` 图元**承载两面交线；`section` 继续保持"平面切实体"的单一语义。`schemaVersion` 保持 `"0.1"`。

### 8.1 分级预览的交互细节（按决策 1 展开）

| 状态 | 几何 | 渲染 | 文案 |
| --- | --- | --- | --- |
| 悬停（未选中） | 悬停对象与指针下可交对象的交线 | 虚线、低不透明度（0.35）、无标签、`pointer-events: none` | 底部状态栏一句：「这两个对象相交，点击可创建交线」 |
| 选中两个对象 | 同上（结果一致） | 虚线、强调色、更高不透明度（0.8）、带分类/段数标签 | 画布标签 + 属性区操作：「创建交线图元」 |
