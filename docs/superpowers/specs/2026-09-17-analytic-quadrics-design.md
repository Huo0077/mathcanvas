...
> **状态**：设计已与用户确认（2026-09-17），**实施已完成并交付（8 片，用户选定"就此收尾 A1"）**——逐片证据与门禁见 `docs/project-progress.md` 的「解析二次曲面 A1」一节。用户口径："我不要一个逼近的圆，我需要一个真的圆，这个曲面的相交太难受了。"
...
- **解析展开（原 §5.5 第 8 项）延后**（2026-09-17 决定，实施第 6 片时评估）：展开的**多边形版与解析版差别只有 0.07%**——圆柱侧面展开成 48 个矩形时，周长比是 `n·sin(π/n)/π = 0.999287`，也就是"宽度差 0.07%"；而投影那一项是**定性差别**（斜看圆得到的是真椭圆还是 48 段折线）。同样的工作量，投影的用户可见价值高一个量级，因此本轮先交付投影，解析展开留作后续（要新做一套展开布局与渲染，不是数字替换）。
...
| 8 | 展开 | 圆柱 → `2πR × h` 矩形 + 两个圆；圆锥 → 扇形（母线长 `l = √(r²+h²)`，圆心角 `2πr/l`） | **延后，见 §4**（多边形版与解析版宽度只差 0.07%，价值低于投影那一项） |
| 9 | SVG 导出 | 椭圆导出 `<ellipse>`；`circle3` 与解析截面按真曲线导出 | 导出串含 `<ellipse`；R=2 圆的半径属性 = `2·scale`（**口径澄清**：2D 画布的 SVG 导出器**故意不导出 3D 图元**——`circle3` / `section` 一律返回空串，3D 工作区禁用投影导出且有 e2e 钉住；所以"`circle3` / 解析截面按真曲线导出"在这条路径上**不适用**，工程制图导出的圆走第 7 项的投影真椭圆。抛物线 / 双曲线**保持折线**：SVG 没有这两类图元，`<path A>` 的圆弧也表达不了它们，硬套就是不精确） |
