...
> **状态（2026-09-17）**：**已交付**——四件事（约束轨道 / 拖动旋转 / 测量数字常驻 / 立体几何 UI 与平面几何对齐）全部实现并过了全套门禁，7 个切片逐片的 RED→GREEN 证据、偏离与如实记下的限制见 `docs/superpowers/plans/2026-09-17-3d-tracks-rotation-and-measurement-labels.md` 与 `docs/project-progress.md` 的「立体几何最后一轮」一节。用户已确认方向（2026-09-17，四个问题分别选定推荐项 A/A/A/A）。**一处按建议调整并已获"继续"确认**：旋转手柄用**世界轴三色环**（X 红 / Y 绿 / Z 蓝，与坐标轴同色），而不是"屏幕法向 / 水平 / 竖直"——理由见 §3.2。
...
| 角的**数值单位** | **已按用户决定统一到弧度**（2026-09-17："改成弧度"）：原先平面测量的 `value` 是弧度（单位 `rad`）、3D 的角是度（单位 `°`），两边各自与自己的属性栏一致却彼此不一致。现在 `measureAngle3` 与二面角都产弧度、单位 `rad`（二面角的说明文字同步换算），画布、属性栏与导出说的是同一个数。**仍然按度显示的只有"输入"与"示例"**：属性栏的旋转角度 / 剖切面 ±15°、旋转环的 15° 吸附，以及 3D 的「测量二面角」演示卡（坐标轴夹角的示例，不是测量）。 |
