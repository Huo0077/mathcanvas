...
**最后更新：** 2026-09-15
**当前阶段：** P1 数学内核、P2 交互和 P3 函数分析已完成，P6 v1 基线 + P6 v2 十个切片完成；P6 v3 立体几何可用性修复完成 5/7，剩余二面角可发现性与测量可视化
**总体状态：** 开发中
...
| 4 | 无法构建平面，更不用说合适大小的平面 | `threeScene.tsx` **完全没有 `plane3` 渲染分支**（创建成功但画不出来）；默认点 `(n%3)*2, floor(n/3)*2, z=0` 让前三点共线（建平面必然报 `plane3 points are collinear`）且全部落在 z=0；界面无任何操作引导；相机固定 16 单位，1 单位图形打开即是一个点 | ✅ 已完成（待审查） |
| 5 | 无法测量二面角，或者测量方法不明确 | 功能本身正确（实测四面体二面角 **45.000°** 与手算一致、状态 valid、画布有标记），纯**入口不可发现**：需在代数区展开拓扑后 Shift 选两个面 | ⬜ 待修复 |
| 6 | 测量结果无法可视化清晰地表现在画布中 | 二面角标记的法向线用固定世界长度，比图形还长、戳出画面；测量数值只在右侧面板显示 | ⬜ 待修复 |

### P6 v3 遗留与待决

- ⬜ **5 号**：让"选中两个面 → 二面角"可发现（画布直接点选面，或在属性面板给出明确的下一步提示）。
- ⬜ **6 号**：测量标记尺寸改为屏幕恒定（与顶点手柄同一套做法）、测量结果直接标注在画布上，并一并处理下面的选中高亮问题。
