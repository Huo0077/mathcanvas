...
- **回归**：全量单测 **100 文件 / 1095 用例通过**（起始 98/1070，本轮新增 25 例）；`typecheck` 4 个 workspace 全过；`lint` 0 error、15 条 warning（起始 16）；生产构建通过；Playwright **82/82 通过**（起始 79，新增 3）。
...
> **当前基线（唯一权威，2026-09-17 在"交面/交线作为独立图元"完成后实测）**：`npm.cmd test` **100 个测试文件、1095 个用例通过**；4 个 workspace 类型检查通过；ESLint **0 error、15 条 warning**；生产构建通过（Vite 仍提示主 bundle 超过 500 KB）；Playwright **82/82** 通过。
> 下面按时间倒序列出各轮实测快照（数字是**当时**的取值，用于追溯与对比，不代表当前门禁）；例如 68 文件 / 698 用例与 Playwright 47/47 属于 2026-09-16 的 Task 15-18 那一轮。

- **交面/交线作为独立图元（2026-09-17，提交 `f2d2476`（内核）/ 本轮提交（预览·图元·检查器））**：①内核 `intersectConvexPolyhedra3`（凸实体半空间裁剪）②画布**自动枚举**所有两两交线/交面（AABB 预筛 + 几何签名缓存 + 布尔配额）③点击创建 `intersectionSolid`（DSL/schema/scene-graph 重算与级联/半透明渲染/检查器读数）④`intersectionLine` 检查器属性块。本轮起始 **98 文件 / 1070 用例** → **100 文件 / 1095 用例**；lint warning 16 → **15**；Playwright **79/79 → 82/82**。过程中由 e2e 探针定位并修掉"点交面正中因命中一条棱而失效"的真实交互缺陷（改为只有顶点手柄优先于预览）。
