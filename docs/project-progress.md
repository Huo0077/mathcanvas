...
> **当前基线（唯一权威，2026-09-17 在"全身大体检（十批）+ e2e 构建修复"之后实测）**：`npm.cmd test` **107 个测试文件、1194 个用例通过**；4 个 workspace 类型检查通过；ESLint **0 error、14 条 warning**；生产构建通过（Vite 仍提示主 bundle 超过 500 KB）；Playwright **83/83** 通过（**现在真的跑的是当前工作区的构建产物**，见下）。
