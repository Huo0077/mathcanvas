...
**最后更新：** 2026-09-17（**五条浏览器反馈修复**：圆柱 / 圆锥默认分段数 24 → 48；动点新增「实体内」约束（可在立方体内部自由移动、越界夹回表面）；修掉依赖图缺边导致"约束动了动点却留在原地"的真缺陷；按用户要求删掉「动效演示」栏。同日更早：**交点 / 交线 / 交面三个独立图元**（自动枚举 + 点哪块建哪块 + 填充色可改 + 交线 / 交点各自成图元）、平面画布拖动动点不再卡顿、浏览器实测反馈的三处修正、3D 视口与几何内核重构、平面几何动点系统）
...
> **当前基线（唯一权威，2026-09-17 在"五条浏览器反馈修复"之后实测）**：`npm.cmd test` **103 个测试文件、1136 个用例通过**；4 个 workspace 类型检查通过；ESLint **0 error、15 条 warning**；生产构建通过（Vite 仍提示主 bundle 超过 500 KB）；Playwright **82/82** 通过。
