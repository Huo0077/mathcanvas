...
| ④ 代理安全测试拒绝 cross-origin / 任意 URL / 过期 profile / 超大 body | 🟡 一半 | 判据层 16 例全过（含计划点名的九种情形）；**真服务器未写**（`tokio` 依赖为 0，`model_run` / `proxy_session` 两个 IPC 命令不存在） |
...
| 1.4 provider 适配器与事件归一 | 🟡 | **`ProviderAdapter` trait 本体未写**（形状取决于 Task 1.5 的传输层）；能力证据探针未写 |
| 1.5 回环代理与传输安全 | 🟡 | **真服务器**（`bind(127.0.0.1:0)` + 路由分发 + 流式转发）；`model_run` / `model_cancel` / `proxy_session` 三个 IPC 命令 |
| 1.6 SQLite 仓储 / CAS / 崩溃恢复 | 🟡 | `.mcanvas` 打包导出/导入；附件两阶段写与孤儿回收（表已建好，blob 目录那一半没写） |

**一句话结论**：G1 的**六个任务主体都在且都被测透**（Rust 88 例 + 单测 2113 例），**已有一条端到端链路**（持久化）；离"完成"还差 **G1 收尾三件**（真代理服务器 + `ProviderAdapter` trait、能力证据探针、`.mcanvas`/附件）+ **Gate ② 的手动重启验证**。
