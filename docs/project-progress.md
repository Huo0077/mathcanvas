...
**最后更新：** 2026-09-17（**交面/交线作为独立图元**：画布像平面画布那样**自动枚举**所有两两交线与交面（AABB 预筛 + 按来源几何签名缓存 + 布尔交集配额），点击虚线建交线、点半透明面片建全新的 `intersectionSolid` 交面图元（进对象列表、随来源重算、参与删除级联、检查器给出体积/表面积/来源），并补齐 `intersectionLine` 的检查器属性块。同日更早：**平面画布拖动动点不再卡顿**（交点预览改为增量，76.5ms/次 → 0.12ms/次，浏览器实测 60fps）、浏览器实测反馈的三处修正（剖面预览只画交线与交点、动点绑定提示、网格严格 1:1）、3D 视口与几何内核重构（10 个切片 + 收尾三项）、平面几何动点系统）
...
### 交面/交线作为独立图元（2026-09-17 已完成）

- **用户要求**（原话）：「还需要优化一下交面交线的问题，我需要交面交线作为单独的图元，ui操作逻辑参考平面」。追问后确认两点语义：**触发方式照平面画布**（场景里凡是相交的对象都自动显示可点击的虚线交线/交面，点一下就创建独立图元，不需要先选中两个对象）；**「交面」= 两个实体重叠区域的整体表面（布尔交集）**，不是"平面切出来的那个截面"。
- **切片 1：内核布尔交集** `packages/geometry-kernel/src/boolean3d.ts`（已完成，提交 `f2d2476`）。
  - 做法：凸实体 = 一组半空间的交集（每个面给一个半空间），用对方的每个面平面**依次裁剪**自己——凸体被平面裁剪后仍是凸体，算法简单、数值可控。切口面按平面内角度排序后补上，顶点按模型尺度量化去重，面按"顶点集合"去重。
  - 输出：`{ status, vertices, faces, volume, area, explanation, diagnostics }`，status 区分 `polyhedron` / `flat`（只在一个平面区域相接）/ `segment` / `point` / `none` / `insufficient-data`（**非凸输入不做近似**，直接给诊断）。
  - RED→GREEN：`boolean3d.test.ts` 7 例先全部因模块不存在失败；实现后 7/7 —— 两个错位立方体的交集是精确的 0.5³（体积 0.125、6 面 8 顶点）、包含关系时等于小立方体（体积 1）、相切时报 `flat` 且面积 1、不相交报 `none`、**交集每个顶点都落在两个输入实体内部或表面上**（半空间判据）、L 形非凸棱柱报 `insufficient-data` 且说明里出现"凸"、退化输入报 `insufficient-data`。
...
- **切片 2：画布自动枚举所有两两交线 / 交面**（已完成）。
  - `apps/web/src/intersectionPreviews3d.ts`：`computeIntersectionPreviews3d(document, { previous, maxSources, maxSolidPreviews })` 枚举**顶层实体**（`cube/pyramid/cylinder/cone/polyhedron3`，排除模板物化出来的"影子"多面体）的两两组合，先用**世界 AABB** 筛掉不相交的对，再算交线（`intersectFaceSets`）与交面（布尔交集）。
  - 交线与交面是**两份独立预览**：`pair:<a>|<b>:线` 与 `pair:<a>|<b>:面`，各自的 key、各自的命中区、点击创建各自的图元。完全包含时只有交面（两个表面根本不相交），只贴面时交面是一块面积不为 0 的平板。
  - 自动枚举**只覆盖顶层实体**：按住 Alt 选中两个**面**（或面 + 实体）时，交线预览仍走**选择驱动**的老路径（`toSelectionLineScenePreview`），并与自动预览按同一套 key 去重——状态栏说"点击即可创建"时画布上必定有那条可点的虚线（否则就是"提示说有、却点不到"）。交线的用户可见名称统一成「交线 · N 段」（此前选择驱动路径叫"面交线"，同一条线两个叫法）。
  - **性能**：每一对按"解析后的几何签名"缓存（`JSON.stringify(topology)`），只动一个实体时其余对直接沿用上一次的结论；单次扫描最多算 12 个布尔交集（交线不受限），超出记入 `truncatedPairs` 并由状态栏说明"另有 N 处只画了交线"。候选上限 24 个实体 / 120 对。
  - RED→GREEN：`intersectionPreviews3d.test.ts` 7 例（先因模块不存在失败）——不选任何东西就枚举出交叠对、相交的一对同时给出交面与交线、包含关系只给交面不编交线、包围盒不相交的对直接跳过、签名未变时 `computedPairs=0 / reusedPairs=1` 且结果逐字节相同（来源移动后重算且体积跟着变）、布尔配额生效（3 对只算 1 对、`truncatedPairs=2`、交线仍是 3 条）、面/平面/模板影子都不是候选。又用**变异测试**确认了配额那条断言真的会失败（把配额判断短路后该用例转红）。
  - 场景侧 `ThreeScenePreview` 从"单份预览"扩成**列表**（每份带 `key`）：`threeScene.tsx` 按 `preview:<key>` 增量同步，指针命中按射线取最近的一份（同一位置同时命中交面片与交线时**优先交线**：细目标被特意指到就是在要交线），悬停高亮改成**就地改材质**（不进内容签名，悬停不再重建场景）。
- **切片 3：点击创建交面图元**（已完成）。
  - DSL 新增 `intersectionSolid`（`vertices` / `faces` / `volume` / `area` / `status` / `diagnostic`，全部由来源重算，界面不给手改）+ schema 校验（来源必须是两个实体、顶点有限、面环下标不越界、体积与面积有限非负、状态在枚举内）。
  - `scene-graph`：`recomputeIntersectionSolid` 走内核布尔交集，`polyhedron`/`flat` 可见、`segment`/`point`/`none`/`insufficient-data` 不可见并给诊断；进依赖图（`sourceIds`）与**删除级联**（删来源连带注销）。渲染成半透明面片 + 实线描边（`createIntersectionSolidGroup`），检查器给出「来源 A / 来源 B / 体积 / 表面积 / 面数 / 顶点数 / 状态」。
  - RED→GREEN：`packages/scene-graph/src/intersectionSolid.test.ts` 6 例（先全部失败）——错位立方体交集体积 32、表面积 64、面 6、顶点都落在交叠区间内；完全包含时等于里面的立方体（8 / 24）；只贴面时报 `flat`、体积 0、面积 16 且**仍然可见**；不相交时 `none` 且诊断含"没有重叠"；来源缺失 / 来源不是实体都给诊断；依赖索引认得 `sourceIds`、删来源连带注销。`apps/web/src/threeIntersectionSolid.test.ts` 3 例（先失败）钉住画法：6 块面片可拾取、半透明、有描边、选中更实、空交集不画占位。
  - e2e `e2e/three-intersection-previews.spec.ts`（配 `e2e/fixtures/overlapping-cubes.mgeo`）3 例：①不选任何东西就有 `data-preview-count=2`、交出 `pair:cube-a|cube-b:面` 与 `:线`、不含远处的 `cube-far`，状态栏说"已自动标出 1 处交线、1 处交面"；②把指针移到交面片上，`data-preview-hover-key` 是该面片、状态栏出现"布尔交集"，点击后对象列表出现"交面 1"、检查器给出体积 `32.000` 与来源 A/B；③在交线那一段上点击创建"截线 1"并显示段数 / 总长度。
- **切片 4：交线图元的检查器属性块**（已完成）：`intersectionLine` 现在显示「来源 A / 来源 B / 段数 / 总长度 / 状态」（状态用给人看的说法：有交线 / 无交线 / 数据不足），有诊断时一并显示。
- **过程中修掉的两个真实交互问题**（都由 e2e 探针暴露，不是猜的）：
  - **点交面正中没反应**：粗拾取恰好命中一条从旁边穿过的棱（`data-pick-readout` 读出 `edge|...|precise|hover|behind`），而原规则是"命中点或棱都不让预览抢"。改成**只有顶点手柄优先**（它是可拖的交互控件，被交面盖住时用户仍然是在抓手柄），棱不再优先——预览的命中区就是画布上真画出来的那份几何，指针落在它上面就是要创建它。
  - **e2e 投影必须先固定相机**：自动取景会在内容同步之后调整相机，用"同步那一刻"的相机读数把世界点换成像素，细目标（交线）会差出十几像素。用例改成先点「重置3D视角」再投影（顺带让自动取景不再抢镜头）。
- **状态栏**：新增 `resolvePreviewInventoryPrompt`——画布上自动铺开的交线 / 交面必须被说出来（用户反馈过"画布上有东西却完全没有任何提示"）：`已自动标出 N 处交线、M 处交面：点虚线创建交线图元，点半透明面片创建交面图元`（配额截断时补"另有 K 处只画了交线"）；交面的悬停说明明确点出"这块半透明面片是两个实体的公共区域（布尔交集）…它的边就是交线、顶点就是交点"。
- **切片 5：只读复查后的五处修正**（已完成，复查由独立子代理执行、只读不改文件，我逐条复核后修的）。
  ...
  2. **截断计数与措辞**：原来只在计算路径自增、且与"有没有交线"绑定 → 完全包含（两表面不相交）的被挤掉对**完全不计数**（静默），而 `MAX_PAIRS` 分支实际连交线都没有、文案却说"只画了交线"（与事实相反）。现在 `truncatedPairs` 无条件计数、每次扫描都报，措辞改成"另有 N 处这次没算交面（交面一次最多算 12 处…）"；另加 `droppedPairs` 单列"超出单次扫描对数上限、连交线都没画"。
  3. **棱不该整类让位给预览**：上一版为了修"点交面正中没反应"改成"只有顶点手柄优先"，代价是**确实压在预览前面的一条棱**再也选不中。现在把取舍抽成纯函数 `previewBeatsPick`（`threePicking.ts` + 单测）：顶点手柄永远优先（截面预览例外），**棱只在指针确实压在它上面**（离射线 ≤ 1/4 容差）时才优先——实测那次是射线在容差内擦过一条棱（`0.6 × 容差`），仍然判给预览。
...
- **回归**：全量单测 **101 文件 / 1105 用例通过**（起始 96/1070，本轮新增 35 例）；`typecheck` 4 个 workspace 全过；`lint` 0 error、15 条 warning（起始 16）；生产构建通过；Playwright **82/82 通过**（起始 79，新增 3）。
- **仍未做（明确记录）**：①布尔交集目前只对**凸**实体成立，非凸输入给诊断而不是近似（L 形棱柱已在测试里钉住）；②交面预览的配额是硬编码 12，实体多且两两相交时后面的对只画交线（状态栏会说明），要做"按音量排序 / 视锥内优先"需要另开一片；③交面图元目前不参与 `visibleSolids` 的展开 / 二面角测量。
...
> **当前基线（唯一权威，2026-09-17 在"交面/交线作为独立图元"完成并复查修正后实测）**：`npm.cmd test` **101 个测试文件、1105 个用例通过**；4 个 workspace 类型检查通过；ESLint **0 error、15 条 warning**；生产构建通过（Vite 仍提示主 bundle 超过 500 KB）；Playwright **82/82** 通过。
> 下面按时间倒序列出各轮实测快照（数字是**当时**的取值，用于追溯与对比，不代表当前门禁）；例如 68 文件 / 698 用例与 Playwright 47/47 属于 2026-09-16 的 Task 15-18 那一轮。

- **交面/交线作为独立图元（2026-09-17，提交 `f2d2476`（内核）/ 本轮提交（预览·图元·检查器））**：①内核 `intersectConvexPolyhedra3`（凸实体半空间裁剪）②画布**自动枚举**所有两两交线/交面（AABB 预筛 + 几何签名缓存 + 布尔配额）③点击创建 `intersectionSolid`（DSL/schema/scene-graph 重算与级联/半透明渲染/检查器读数）④`intersectionLine` 检查器属性块。本轮起始 **96 文件 / 1070 用例** → **101 文件 / 1105 用例**（含复查修正新增的 10 例）；lint warning 16 → **15**；Playwright **79/79 → 82/82**。过程中由 e2e 探针定位并修掉"点交面正中因命中一条棱而失效"的真实交互缺陷（改为只有顶点手柄优先于预览）。
