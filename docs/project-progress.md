...
>
> 这份文件保留**过程**：每一轮的 RED→GREEN 证据、被实测推翻的方案、当时的门禁数字、
> 误报与如实缺口。**下面任何数字都只代表"写它的那一刻"**，不要当作当前值。
>
> 纪律：引用本文件一律用**小节标题**，不写行号 —— 行号会随任何一次编辑静默失效。

> 下方 2026-09-22 的"最后更新 / 当前阶段 / 总体状态"三行保留为**当时快照**，不再维护。

**最后更新（归档快照，2026-09-22）**：（**四份 2026-09-21 计划全部落地，并完成一轮收口**：①统一 Solid 与拉伸式 Prism ②Reactive DAG ③Agent DSL 与参数审计 ④多会话上下文；收口轮把 Solid 派生读数接进属性面板与模型观测、把编译阶段的一次性修复回路接通、并补齐会话事实的 `stale`/`retracted` 与会话摘要的落盘诊断。四个切片各自实现 + 独立评审 + 修复轮，最终 **215 文件 / 2560 用例（+1 todo）**、typecheck 6 个 workspace exit 0、lint **0 error / 14 warning**、**e2e 140/140**、Rust **219 例 + 3 ignored**；**已推送到 `origin/main`（远端 `42373eb`）**。**另附：四路外部审查在计划任务之外查出 2 Critical / 14 Important / 16 Minor 缺陷**（**随后已全部处理：2 Critical + 14 Important 修复，Minor 16 条中 15 条修复、1 条如实标注尚未接线**），逐条见下文「已知缺陷清单（2026-09-22 四路外部审查）」。**随后同日开修：两条 Critical 与六条 Important 已修复 —— X1（SQLite 从来不是文档真源：探测用的是刚生成的新随机 id）、X2（一次性同意的 CAS 在"点确认"时才取样，用户看预览期间的编辑被静默合并）、D1（导入的附件字节没登记引用，被孤儿回收删掉）、D2（`read_attachment` 的任意文件读取）、D3（`import_package` 逐份开事务 ⇒ 部分导入）、G1（外接球枚举全部 C(n,4)，且属性面板每次选中都算整篇文档 ⇒ 用户可达的分钟级卡死）、G3（`facesOutwards` 只看头三点，前三点共线时面被翻反）、G4（退化判据的容差其实不相对，1e-5 量级的合法三角形被判成退化）、G2（带 `plane` 字段成了绕过棱柱几何校验的通行证，导入与保存两条路径一起漏）、S1（一次交互压两条撤销记录，一次 Ctrl+Z 落在用户没见过的中间态）、S2（删除棱柱只删掉 `polyhedron3`，26 个派生拓扑留在画布上）、A1（"无需改动"被报成失败，协调器的 `no_change → completed` 成了死代码）、A3（读会话记录抛错让整轮在协调器启动前卡死）、S3（平面动点的宿主路径从不校验存在性，悬空 `pathId` 让点静默冻住而界面仍显示"绑定在路径上"）、A2（引用解析只认第一个字段，第二个引用永远指不到同一份计划里的别名，唯一一次修复还被指向错的字段）、A4（用户确认之后账本永远停在"等用户确认"，即使文档真的提交了），各自取到 RED→GREEN 与变异检查证据 —— **至此 2 Critical + 14 Important 全部修完**；随后进入 Minor：**内核那 3 条也已修**（`sampleLocus` 的 `maxJump` 方向反了、`liftPrismBasePolygon` 的基底是左手系且把坐标转置、`buildPrism` 对顺时针底面静默产出里外翻转的实体），随后 **scene-graph 3 条**、**Agent 3 条**与**桌面/会话 5 条**也修掉（棱柱描述符在搬动后重检、`commitTransaction` 校验结果文档、批量改样式与批量显隐同一条锁策略、`beginStage` 真的被调用、上下文预算真的计费、矛盾检测覆盖 `parameter.set`；`create` 不再把任何插入失败说成"已存在"、`upsert_fact` 采纳 `created_at`、尺寸闸门的数字与计数方式与 Rust 对齐且摘要上限收敛成一个数、`archive()` 不再推进 `updated_at`、密钥字段守卫递归进数组），**Minor 只剩 2 条**。见下文「修复轮 A」「修复轮 B」及其三次续篇。）。下面这一段是上一批（**非空画布上的两处必然失败：id 分配器 + 规范化哈希**。用户截图里那条直四棱柱请求以 `compile_failed: duplicate object id` 结束，修掉之后**当场暴露出第二个缺陷**（此前被第一个挡着看不见）：`canonicalContentHash: unsupported value of type undefined`。①**分配器占用集**：`createIdAllocator()` 的计数器从 1 重数、不知道文档里已有哪些 id —— 画布上已有 `solid-1`，新动作又被发成 `solid-1`；手工路径的 `nextPrimitiveId` 一直是对的，漂移的只有动作层。②**哈希与 JSON 对齐**：应用启动恢复时会 `migrateLegacySolids`，物化出来的子对象带着 `style: undefined` / `label: undefined`；文档的每一处落盘与比对（JSON、`contentFingerprint`）都当"没这个字段"，只有规范化哈希抛错，于是"画布上有一个立体"就等于"Agent 必然失败"，而且错误信息只有一句内部函数名。两处都按 RED→GREEN 修：**+11 例单测**（分配器 3 / 草稿 2 / worker 1 / 组装运行时 1 / 哈希 3 / 迁移 1）+ **2 条 e2e 回归**（先有一个对象再建同类；刷新恢复之后再起草）。**真窗口实测**（重建 exe → 重启 → 真 DeepSeek）：`再建一个棱长 2 的立方体` → 已提交 → 对象 29；用户原来那条直四棱柱 → **草稿成型**（共 30 个 · 3 条假设 · 0 个页面错误），刻意留给用户确认。**验证**：单测 **193 文件 / 2207 用例**、typecheck 6 个 workspace exit 0、lint **0 error / 14 warning**、**e2e 124/124**、Rust 未改动（193 例 + 1 ignored）、release exe 重建并实跑）。下面这一段是上一批（**G2 接线：`PlannerPort` 接到真实 provider + G1 第十四批：项目包界面入口**。用户口径："继续完成 G1 剩下的两项 + G2 接线"。这一批把 G1 第十三批留下的两条如实缺口一起关掉：①**「使用中」的那一份配置现在真的被规划器消费**——`modelPlanner.ts` 自己现取它（三道检查：有没有桌面外壳 / 有没有选过 / 有没有密钥），按**能力证据**选通道（`json` 已验证才走严格 JSON，否则文本通道），provider 失败**抛**带分类的错、解析失败**原样交出去**让协调器走一次性修复，abort 时真的调 `provider_cancel`；②**附件与 `.mcanvas` 导出/导入有了界面入口**（「项目包…」，与"打开/保存 `.mgeo`"同一组）。附件哈希必须按**原始字节**算（`sha256HexBytes`），否则前端与 Rust 侧会算出两个哈希。**两处靠非测试手段发现的缺陷**：一处 CSS 令牌名写错导致面板背景**静默变透明**（`--color-surface` 不存在，是量 `getComputedStyle` 发现的），一处提示文字原样显示 markdown 记号（看截图发现的）。**如实缺口**：`native_tools` 通道到不了（`provider_run` 契约里没有工具表位置）、附件列表只覆盖本次会话（缺"列举某快照的附件"命令）、**没有对真实 provider 跑过一轮**（本机没有配过任何 profile/密钥）。**验证**：单测 **191 文件 / 2182 用例**（零跳过）、typecheck 6 个 workspace exit 0、lint **0 error / 14 warning**、**e2e 122/122**、Rust **179 例 + 1 ignored**、clippy exit 0、release exe 真的启动过、实拍截图两张（项目包面板 / 桌面外壳窗口）。）。下面这一段是上一批（G1 第十三批：模型服务界面重做成 CC Switch 那种清单。用户口径："像 ccswitch 那样，首先有加号可以添加 apikey，添加完成并且通过验证之后，在界面可以出现刚刚填入的一栏，能同时存在很多栏，并且能够主动在不同的模型中进行切换"。交付：整屏重做的清单界面（加号 → 表单 → 保存即验证 → 那一栏出现；每一栏一张卡片，含四个能力徽章与"使用中"标记）、Rust 侧的"当前使用哪一份"（`active_profile_id` / `select` + 两个命令，7 例）。**切换只动一个字段、不碰修订号**，因为能力证据挂在修订号上，切换若推高修订号会把四个徽章全清掉。三处新判据都落在存储层：切换跨重启还在 / 删掉选中那份时选中被清空 / 旧配置文件仍打得开。**一个自己写出来的 CSS 优先级缺陷靠截图发现**：`data-active` 与 `data-state="unverified"` 各写一条 `::before`，"正在使用的那一栏恰好也还没验证"时会显示成红线。**如实缺口**：`使用中` 还没被规划器消费（把 `PlannerPort` 接到真模型属 G2）。**验证**：Rust **179 例 + 1 例 `#[ignore]`**、单测 **188 文件 / 2134 用例**、typecheck exit 0、lint 0 error / 14 warning、`cargo clippy --all-targets` 零警告、`npm run build` exit 0、**e2e 119/119**、实拍截图两张。）。下面这一段是上一批（G1 第十批：能力证据探针 —— "已验证"真的会变真了**。上一批 provider 会发请求了，这一批回答"**一次成功的请求到底证明了什么**"：四条探针各发各的请求，逐条下结论，而最要紧的判据是计划原文那句"**一次成功的文本 ping 不许把 vision 或 tools 打勾**"。**证据的边界**成了这一批的核心：图片与工具探针被 400 拒 → `failed`（形状是我们拼的，那是要我们改代码）；认证 / 连接 / 没有密钥 / 模型不肯配合 → `unknown`（什么都不说明）。**"这家不支持"与"我们发错了"必须分开**，因为下一步动作完全不同。顺带把请求拼装扩成能带工具表与图片（三家三套形状，逐条断言）。设置界面多了一个**用户按下去才发生**的探测按钮（一次四发真请求，按钮写明代价，没有密钥时禁用）。**验证**：Rust **140 例 + 1 例 `#[ignore]`**、单测 **188 文件 / 2122 用例**、typecheck exit 0、lint 0 error / 14 warning、`cargo clippy --all-targets` **零警告**、`npm run build` exit 0、**e2e 119/119**。**G1 现在只差 `.mcanvas` 打包与附件**（Gate ② 的手动密钥重启验证需要用户本机操作）。同日更早的 G1 各批（1.1 外壳 / 1.2 密钥库 / 1.3 provider 配置 / 1.4 适配器 / 1.5 代理 / 1.6 仓储 / 真实转发）逐条见下方「G1 第一批 … 第九批」。同日更早的 G2 各批（第二十二批至第二十七批：接缝修复 / previewHash 真哈希 / 假设贯通 / 同意不可伪造 / 工具可执行 / 轨迹面板 / worker 信封 / 上下文交给规划器 / 观察者事实文本 / 技能清单 / prepare 已知缺口 / 一次性修复往返）见下方「G2 第二十七批」及更早各节。
**当前阶段（归档快照）：** 四份 2026-09-21 计划（Solid/Prism、Reactive DAG、Agent DSL、多会话上下文）已实现并提交；P0-P6 与 P7 工程制图已完成；MathCanvas 统一 Ribbon UI 基线、后续 UI 优化（Task 7-13）、工程制图视觉重做（Task 14）、工程制图可用性修复（Task 15-18）、圆锥曲线四项修复、功能键操作指引浮层、CAD 2D 绘图交互重做、平面几何动点系统、3D 视口与几何内核重构、封闭曲线绕定点旋转、UI 优化（草稿纸画布）与平面几何元素选颜色均已完成。**2026-09-17 新增两条解析几何交付线并已全部落地**：**A1 解析二次曲面与"真圆"**（8 片；设计 `docs/superpowers/specs/2026-09-17-analytic-quadrics-design.md`）与 **A2 交面按支撑曲面分组 + 真曲面**（5 轮；设计 `docs/superpowers/specs/2026-09-17-intersection-face-grouping-design.md`）——用户口径从"我不要一个逼近的圆，我需要一个真的圆"一路推到"我需要的只是那个相交的曲面，而不是由很多三角形拼出来的"。**随后"立体几何最后一轮"四件事也已全部交付**（7 片；设计 `docs/superpowers/specs/2026-09-17-3d-tracks-rotation-and-measurement-labels-design.md`）：约束轨道（`circle3` 当动点宿主）、拖动旋转（世界轴三色环 + 15° 吸附 + 属性栏角度）、测量数字常驻画布（2D + 3D）、立体几何 UI 与平面几何同一套令牌。平面动点系统按四个维度交付：①约束模型与参数化映射 ②依赖图 DAG 与增量拓扑重算 ③动态测量监听器 ④轨迹采样与消元法隐式化；3D 重构按四个区块交付：①动点宿主约束与渲染管道 ②截面几何 ③Auto-Fit ④生命周期与多解；四者与三区块**全部接进主流程**（不只是内核可用）。**2026-09-18 又完成平面几何切线**（抛物线 / 双曲线 / 圆 / 椭圆的曲线切线，切点可沿曲线拖动或跟随动点）**与动点扩展**（在动点处作切线、以动点为圆心作圆、半径可调且可随动点位置动态变化），并修掉"切线不能拖动"这一现场反馈。**P4 Agent 已按用户要求交付**（G1 + G2：Tauri 外壳 / 凭据管理器 / provider 与能力证据 / 回环代理 / SQLite 仓储 + CAS / 真协调器 + 隔离草稿 + 一次性同意 + 真实模型规划器，见本文件的「G1 阶段状态」表与「G2 接线」、以及最新一批两条修复）；**P5 题图解析仍在排除范围内**。
**总体状态（归档快照）：** 开发中

### 四份 2026-09-21 计划：Solid/Prism、Reactive DAG、Agent DSL、多会话上下文（2026-09-22）

设计真源：`docs/superpowers/specs/2026-09-21-incremental-geometry-agent-and-conversations-design.md`。四份计划各自"实现 → 独立评审 → 修复轮 → 复跑门禁"，本轮共 4 个本地提交（**未 push**）：

| 提交 | 内容 |
| --- | --- |
| `7cff139` | 计划①②③：Solid/Prism + Reactive DAG + Agent DSL（87 文件） |
| `fdb28a8` | 计划④ 任务 1-3：SQLite 会话真源 + 8 条具名 IPC + 类型化客户端 + store 直写（15 文件） |
| `0476321` | 计划③ 修复轮：§8.2 不变量、§8.1 截面边界点、prompt 生产接线、静默保存缺陷 |
| `9461d4b` | 计划④ 任务 4-6：会话上下文注入、摘要与事实、跨会话隔离、载荷扫描 |
| `500b15c` | 计划④ 修复轮：按文档绑定与事实隔离、runId 授权确认、§5.3 预算、按文档摘要 |
| `16a7ed1` | 收口轮：Solid 派生读数进 UI/观测 + 编译修复回路接通 + 会话事实与摘要缺口（42 文件） |

> 提交：`7cff139`（①②③）、`fdb28a8`（④1-3）、`0476321`（③修复轮）、`9461d4b`（④4-6）、`500b15c`（④修复轮两轮）、`16a7ed1`（收口轮）、`63e16ac`/`ca95c51`/`30dda29`（进度文档与复选框裁决）、`42373eb`（外部审查发现与四份原始报告）。计划①③④都经过"独立评审 → 修复轮 → 复审"闭环；**以上提交已全部推送到 `origin/main`（远端 HEAD = `42373eb`，经 GitHub API 复核）。**

**① Solid/Prism（`2026-09-21-solid-prism-implementation-plan.md`）**：`SolidConstruction` 以 `prism` 构造为真源（`base.polygon` + `vector`），规范 §3.2 的 `base.plane` + 二维多边形**在 codec 边界被抬升为世界坐标**后接受；纯函数 `buildPrismTopology` 生成底面/顶面/侧面环与确定性子对象 id（`solidId:v0/e0/f0`），ID 取文档占用集（画布已有 `solid-1` 时新棱柱是 `solid-2`）；`solveCircumsphere3`/`solveInsphere3`/`sectionSolid3` 返回 `exact/undefined/degenerate/approximate` 四态，`solidStatusReport` 把它们暴露到 scene-graph 生产层；旧 template/fromPoints/fromFaces 文档仍可读。评审抓出的 Critical（**零体积棱柱被接受**，即拉伸向量平行底面时写出"纸片"）已修：`degenerate-volume` 诊断 + 回归；一致性测试（`buildPrism` vs `buildPrismTopology` vs 校验器，10 组输入）还揪出老实现里同一处投影缺陷并修掉。

**② Reactive DAG（`2026-09-21-reactive-dag-implementation-plan.md`）**：`packages/geometry-kernel/src/reactive/` 新增纯 evaluator 图（拓扑序增量求值、反向依赖失效、**先拒环再跑 evaluator**、非有限/缺源/退化都是结构化诊断），参数化宿主约束（线段/直线/圆参数、面 `u,v`、体 `u,v,w`）、五心与外接/内切圆、切线/截面/测量/轨迹（自适应采样 + 临时 Trace 不入历史，一次拖动=一步撤销）；2D 拖拽的临时轨迹、轨迹采样、3D 宿主拖动坐标与诊断读数都走图，**没有引入第二份几何真源**。修复轮：`removeNode` 的反向闭包（原来会留下 `status:"exact"` 的过期坐标）、被 `evaluate` 抹掉的非有限参数诊断、3D 绑定参数未纳入引用守卫（能造出存不下去的文档）、单槽图缓存导致有轨迹时每帧重建（改按文档 WeakMap + `data-reactive-graph-reused` 读数）、闭合宿主域语义两份实现（统一 `normalizeHostParameter`）。

**③ Agent DSL（`2026-09-21-agent-dsl-implementation-plan.md`）**：动作/字段审计元数据（必填、`safe_default`/`infer_from_facts`/`ask_user`/`reject`）、六层编译（传输解析→字段审计→引用解析→参数补全→几何语义校验→隔离草稿动作编译，每层拒绝都带路径与 code）、欠定特值选择（**"任意/恒定/定值"保留符号参数**）、一次性修复（只带 code/path/allowedChanges）、版本化系统提示（策略与上下文分离）与"未提供的工具一律拒绝"。评审判 §8 验收未达标并给出 3 个 Critical，修复后：**§8.2 的不变量本来就是错的**（原式是 `sec²θ+csc²θ≈7.77`，且没有任何地方求值它）→ 现在 `OA=3/cosθ`、`OB=2/sinθ`、A/B 作为参数驱动点建出，表达式经 `recomputeDerivedObjects` 在多个 θ 上求值并断言"数值采样不等于形式证明"；**§8.1 的"P 在截面边界上"由文案变成事实**（截平面含底面一条棱，P 在每个参数下都在截面多边形上，浏览器断言改断 `data-binding-parameter` 而非标签）；**prompt 真正接进生产链路**（coordinator → CommitRequest → CommitterAdapter → `DraftStore.stage` → `compilePlan`，以及 worker 的 `GeometryCompileRequest`），符号参数约束才有机会生效。指标修正并钉死：漏参 **3/55 = 0.055**、修复率 0.2、澄清率 0.2、草稿成功率 0.6。顺带修掉 Reactive DAG 切片发现的**静默保存缺陷**：`commitPatch` 现在校验结果文档、`saveDraft` 不再吞掉 `encodeMgeo` 失败。

**④ 多会话上下文（`2026-09-21-conversation-context-implementation-plan.md`）**：迁移 v3 建 `conversations`/`conversation_messages`/`conversation_facts`（含索引，旧库可迁移且不丢文档），仓储有界读（保留最新）、确定性排序、消息 id 幂等、事实必须引用**本会话**的消息；8 条具名 Tauri 命令 + 类型化 TS 客户端（浏览器只在显式 fallback 边界下用 localStorage）；store 先写仓储、成功后才更新内存投影，两个 binding 互不可见，删掉的会话不会从旧缓存复活；`ConversationContext`（binding/摘要/已确认事实/最近消息/场景观察/可选草稿视图）按 §5.3 顺序与预算组装，**一次运行只固定一次并原对象传给修复尝试**；迟到的运行事件写回**原会话**；提交成功写一条 confirmed 事实（文档 generation + 新建对象 id）并在阈值后结构化压缩摘要（原始消息不删），discard/no_change/编译失败不写事实；新增载荷扫描测试断言落盘内容不含密钥形状字符串、候选文档字段与 `AgentMessage` 允许集合之外的键；`e2e/conversation-isolation.spec.ts` 走 A→提交→切 B→隔离→切回 A→继续引用。

该切片独立评审给出 2 Critical / 4 Important，修复轮把 §9 那两条门禁真正补上：**①"一个会话绑一个文档"在生产里曾经是空的**（`setBinding` 除测试外无人调用，所有会话都落在常量 `{local,local,conics}` 上，于是 geometry3d 会话的已确认事实会被注入 conics 文档的运行里）→ 现在 `useAgentDocumentBinding` 跟着活动文档/工作区绑定（并区分"用户切工作区"与"Agent 为完成计划自己切工作区"，后者不夺走用户的对话与确认面板），事实带 `documentId` 并在注入时按文档过滤，**摘要是按文档存放的册子**（`ConversationSummaryBook.byDocument`），旧扁平摘要一律当"没有摘要"而不猜测；**②确认面板曾经可能提交另一个会话的草稿**（`pendingCommit`/`runtime` 是模块级单槽且不校验会话）→ 运行状态改为按 `runId` 索引，`runId` 写进该轮消息并一路传到确认/丢弃/停止，同意检查带上 `conversationId`，不匹配返回 `stale_conversation`；另外修掉：规划上下文改读**固定**会话（原来在 await 之后读活动会话，B 的历史会喂给固定到 A 的运行）、浏览器 fallback 补上与 Rust 一致的密钥前缀与尺寸闸门、§5.3 的 20/15/20/35/10 预算（原来事实带不设上限且渲染只取 16 条，长事实集会把摘要与最近消息挤空）、摘要改为在预算内重建合法 JSON（原来会把 JSON 从中间切断）、被拒绝的提交不再销毁运行（重试可用）；e2e 补上"引用 A 里真实对象、且不泄漏到 B"的断言。

**收口轮（2026-09-22，`16a7ed1`，42 文件）**：四份计划落地后按其自身"如实缺口"再做一轮，并由一个独立评审复核该轮（0 Critical / 3 Important，全部修复）。①**Solid 派生读数进到用户与模型眼前**：3D 属性栏新增「派生读数」块，按 `solidStatusReport` 显示外接球/内切球/截面各自的 `exact`/`undefined`/`degenerate`/`approximate` 与原因（`approximate` 与 `exact` 在状态属性、文本与配色上都可区分，`undefined`/`degenerate` 一定打印原因）；同一套"归属规则"抽成 scene-graph 的 `topologyOfEntity`，属性栏与既有 `templateTopology` 共用，因此**顶点编辑把构造记法改写成 `fromFaces` 之后面板不再静默消失**（评审 I2）；观测层把同样的状态（默认 12 / 上限 24，常量只有一处）带进 `ModelContext.derived` 与会话观测副本，超限时在模型看得见的地方给出 `truncated_derived`（评审 I3，此前默认只取 12 使告警永不触发），提示词升到 v6 并写明四态语义；选中的截面显示**它自己那一次**的读数（读数带 `sourceId`，React key 唯一，截面行用自己的标签）。②**编译阶段的一次性修复回路接通**：编译器自己的结构化 `RepairRequest`（reason / errors 的 code+path+detail / allowedChanges / attempt）连同诊断与失败前假设，经 `draftStore → committerAdapter → coordinator` 交给规划器，取代协调器自己拼的那一份；每轮最多修一次、与运行预算共享、第二次失败即以 failed 结束、提交的是**通过编译的那一份计划**（评审 M4 用"改坏提交载荷/暂存载荷"两次反向验证）；提示词在既有"上一轮输出没有被接受"下逐条打印诊断/允许改动/假设，并在**产出侧**加了 `isEchoableName` 闸门，使散文形状的字段名不能借路径或详情回显进提示词（评审 M3）；`contextLimits.derived` 现在可达并下沉到会话副本。③**会话缺口**：事实新增保守的 `stale`/`retracted`（只在文档能证明时降级——被引用对象全没了、或文档退回事实写下之前那一版——证据记进 `valueJson.invalidation`，读长期记忆前重判一次，撤回后不进任何一轮）；摘要册在 16K 内**必定装得下**（先丢最旧别份文档、正在写的那份留到最后）且不再静默——落盘诊断改按"会话 id + 消息 id"落点，因此能活过那条会销毁 run target 的回执（评审 I1），同一处修复还发现并修掉第三个被同样吞掉的诊断（`[ledger] append failed`）；用户提问消息行现在真的带上 `document_generation`（没装读口时仍为 NULL 而不编数）；两个 Rust 测试注释去掉 `unused_doc_comments` 警告。该轮还披露并完整恢复了一次事故：一个 worker 的 PowerShell `Get-Content | Set-Content` 变异检查把 `coordinator.ts` 按 ANSI 重解码、毁掉了其中文注释，随后用 HEAD blob 逐字节还原（`git hash-object` 与 `git rev-parse HEAD:…` 比对一致）、用编辑工具重放该切片的改动，并全树复扫 **42 个改动源文件 0 个替换字符**；五道门禁均在恢复之后重跑。

**如实缺口（未声称完成）**：①§8.1 的 P 只遍历截面的一条边界棱，且"截面不跟随平移后的立体"作为 `it.todo` 钉住；②Reactive DAG 的 2D 预览几何仍由既有 `applyOperation`/`recomputeDerivedObjects` 计算（图负责临时轨迹、轨迹采样、3D 拖动坐标与诊断），**计划 Task 4 的第三条复选框据此保持未勾选**；③会话按文档**绑定/事实/摘要**都已隔离，但"一个文档一条独立对话"仍是产品取舍：Agent 自己切工作区时不改绑，`cad` 工作区同理；④**撤回事实只有 store 层入口**——该轮写入面不含 `components/**`，而仓库里还没有渲染事实列表的组件，所以用户目前点不到"撤回"；⑤欠定特值选择在生产路径上目前只有棱柱可达；⑥三角形五心/球体动作仍登记为 `unsupported_action`（拒绝文案准确）；⑦模板立体的**单顶点编辑**会被拒绝（宁可拒绝也不静默落盘校验不过的文档），恢复需要三角化/放宽共面判据；⑧`approximate`/`degenerate` 在今天的核里不可达（只由单测以生产形状的条目钉住），且"观测 → 提示词"这一段是在三层里证明的（e2e 诊断行、运行时断言、提示词渲染断言），**未对真实 provider 载荷断言**；⑨编译细节里仍有同类"模型自选 id 回显"通道（`no object <id>`、`invalid_json` 片段）未加闸，模式已建立，留给后续轮次；⑩Solid 观测的截断告警在两个 builder 各自产生时可能出现两条**不同数字**的同名告警（完全相同 code+detail 的已去重）。
...
### 评审方案落地（2026 年本轮，按《MathCanvas 项目梳理与优化建议》）

评审基线是 `3d083360`。以下按方案的编号逐条记录，**每条都附可复核的读数**。

**方案 1（P0）统一实体构造与拓扑物化 —— 已完成。** 现场是"Agent 造的立方体改朝向、画布不动"。根因比方案里写的更具体：模板实体在文档里是**两件东西**（用户编辑的参数化图元 + 由它物化出来的一族拓扑），手工按钮 `App.tsx` 用 `buildSolidTemplate` 一次落盘两件，而 `solid.create_template` 只落盘了第一件；没有 `polyhedron3` 拓扑时渲染落到"直接画模板"的分支，那条分支读尺寸与位置、**不读 `rotation`**。三处改动：①动作层加唯一入口 `compileTemplateSolid`（刻意**不传**自定义 `BuilderContext`，与 `syncTemplateTopology` 的默认命名逐字一致 —— 两套命名分叉时顶点永远回填不上，这是第二个坑）；②手工按钮改调同一入口，两条路真正合流；③`DEFAULT_SOLID_SEGMENTS` 下沉到内核（此前 48 在三处各写一份）。回归用例 `materialises the template topology so a later rotation actually moves the vertices` 逐顶点比对"绕模板中心转 45°"的解析值。

**方案 1 的验收标准：手工与 Agent 必须落盘同一种东西（2026 年本轮，已直接测过）。** 评审方案 1 的验收原话是"Agent 和手工分别创建**同参数**的立方体、棱锥、圆柱、圆锥，文档中均有实体与对应拓扑，**数量与引用关系一致**"。此前**没有任何一条用例把两条路的产物放在一起比** —— 已有用例分别检查"动作编译产出什么"和"手工由 `compileTemplateSolid` 构造"，但"一致"这件事本身没被钉住。
现在有四条参数化用例（`actions.test.ts`），逐字复刻两条真实路径：手工按 `App.tsx` 的 `addSolidTemplate` 两步（`compileTemplateSolid(id, primitive)` → `[primitive, ...result.primitives]`），Agent 走 `compileActions` 的 `solid.create_template`，然后断言**逐字节相同**（`toEqual` 整个图元数组），并额外比对那只 `polyhedron3` 的 `vertexIds` / `edgeIds` / `faceIds`。
"逐字节相同"比"都能画出个立方体"强得多：**它会因为某一侧换了命名、漏了某一族子对象而红** —— 那正是 P0 之前的状态（只在动作侧落盘模板图元、没有拓扑）。参数取的是手工按钮真实的默认落点（±5 象限、棱长 4 / 半径 1.5），所以这条不只是"两个函数调同一个东西"的同义反复。

**方案 1 的收尾核对：计数口径（2026 年本轮）。** P0 让 `solid.create_template` 一次落盘整族拓扑之后，一个立方体在文档里是 **28 个图元**，于是要回答一个产品问题："确认面板说'会新增 28 个对象'，这句话对吗？"为此给 `packages/agent-core/src/draftCounts.test.ts` 补了一条**按真实编译产物**的用例（此前所有夹具都是手写 `tessellation` 标记，**没有任何一条用例喂过真东西**）。
**结论：计数是如实的**——28 = 模板图元 1 + 8 顶点 + 12 棱 + 6 面 + 1 多面体，全部计为 `user`；`derived` 与 `internal` 都是 0。判据不是照抄数字，而是逐类核对：`polyhedron3` 与它的子对象**不在** `DERIVED_PRIMITIVE_TYPES` 里 —— 它们不是"由别的对象算出来所以拖不动"，而是实体本身，在对象树里真的占行、能选中、能删。这条同时是"P0 没把计数搞坏"的回归：哪天 `solid.create_template` 又只落盘模板图元，`total` 会掉到 1，用例立刻红。
（写这条时我自己手算成 27 —— 漏了模板图元。所以最后逐类断言而不是只信一个手算总数。）
**一条留给产品的观察**（不是缺陷）：用户要"一个立方体"，面板会说"会新增 28 个对象"，因为子对象**不按属主实体归并**。计数本身没错（28 个对象确实都会进文档），但"要不要按实体归并着说"是产品判断，与方案 1 里"对象树以拓扑为依据"是同一个问题的另一面。

**方案 2（P1）拆分大文件 —— 已开第一批。** `operations.ts` 从 **2817 行降到 2662 行**，拆出 `solidGeometry.ts`（334 行）：立体与截面的几何（模板 / 物化拓扑 → 顶点 + 面环，供截面、二面角、截面物化消费）。`operations.ts` 用 `export * from "./solidGeometry"` **原样再导出**，跨包调用点一个字都没改。
**拆这一块踩到的两个真坑，记下来给下一批**：①**块搬动必须逐字照搬**。手抄的第一版把 `edgesBetween` 的竖棱条件（`if (offset > 0)`）、`topologyIn` 的"模板记法优先 / `fromFaces` 兜底"、`movedSectionPlane` 的法向归一化全抄错了 —— 三处都是**看着可疑、实则是刻意为之**的判据，重写就等于悄悄回退既有修复。最终这个文件由 `git show <base>:…/operations.ts` 的原字节区间生成并逐字节比对通过（`MATCH`）。②`export * from` **不会在当前模块建立局部绑定**，本文件内部用到的名字必须另外 `import` 一次。
**方案 2 第二批：`PropertiesBar.tsx` 的字段原语（2026 年本轮）。** 先拆"控件层"而不是先拆面板，因为**不管面板怎么拆，字段原语的实现只有一份** —— 先把它拿出来命名，后面拆面板时才有共用的一层可依赖，而不是每个面板各自再写一个 `CoordinateField`。结果拆成两个文件：`components/inspectorFields.tsx`（62 行：`Field` / `InspectorAccordion` / `CoordinateField` / `Vector3Fields` / `CurveTangentAction`）与 `components/inspectorMath.ts`（66 行纯函数：度数↔弧度换算、绕点旋转、直线的斜率 / 倾角 / 长度）。`PropertiesBar.tsx` **1069 → 990 行**。
**为什么拆成"组件 / 纯函数"两个文件而不是一个**：`react-refresh/only-export-components` 要求只有导出组件的文件才能热更新 —— 一个文件既导出组件又导出函数时，改一次函数会让整块属性面板**丢掉状态**。第一次拆成一个文件时 lint 从 14 条涨到 25 条（9 条 fast-refresh + 2 条拆完没人用的 import），按这条规则拆开之后回到 14 条基线。
**方案 2 第三批：`App.tsx` 的文件导出族（2026 年本轮）。** 拆出 `apps/web/src/persistence/fileExports.ts`（131 行）：`.mgeo` 保存 + SVG / DXF / PDF / CSV / PNG 导出，此前是 `App.tsx` 里七个闭包、约 70 行，夹在属性计算与文档恢复之间 —— 而它们与"文档怎么被恢复、选中了什么"毫无关系，只是**读文档、写文件**。`App.tsx` **2000 → 1945 行**。

**方案 2 第四批：`App.tsx` 的 id / 标签分配与键盘判据（2026 年本轮）。** 拆出 `apps/web/src/documentIds.ts`（108 行）：五个 id 分配器（图元 / 分组 / 标注 / 工程标注 / 测量）、两个点标签分配器、以及 `isTextEditingTarget` / `historyShortcut`。`App.tsx` **2000 → 1887 行**（累计）。
**为什么先挑这一族**：它们此前是 `App.tsx` 里的模块级函数，而 `useSceneStore.getState()` 只被**当作类型**用 —— 于是这段规则既进不了别的模块、也没法单独测。更要紧的是它正落在评审方案 1 的主线上：**同一件事有两套实现**，手工路径（这里）扫已有 id 取下一个空位，Agent 路径用 `scene-graph` 的 `createIdAllocator`。这两套漂移过一次就有真实现场 —— 分配器不知道文档里已有什么，画布上已有 `solid-1` 时 Agent 新建的第一个立体又被发成 `solid-1`，整轮以 `compile_failed: duplicate object id` 结束（账本 `run-6-mubf109e`）。搬出来之后它们至少各自可测，将来合并也有明确落点。
**新用例 10 条**（`documentIds.test.ts`，这一族此前**一条测试都没有**）：稀疏占用才是关键（1 和 3 被占时要发 2，只看"有多少个"会撞上 3）、五类各自编号互不干扰、可选字段缺失不抛错、标签 A…Z 用尽后退成 `P27`、平面点与空间点同一条规则、以及键盘判据（Cmd/Ctrl 变体、Shift 转 redo、Ctrl+Y、Alt 组合不算、**焦点在输入框里时不许触发撤销**）。
**搬动时抓到一个真缺陷**：`isTextEditingTarget` 的返回类型是 `boolean`，但实现里 `HTMLElement.isContentEditable` 在 jsdom 与部分 WebView 里是 `undefined` 而不是 `false` —— 于是整个表达式可以返回 `undefined`。调用方只要写 `=== false` 判断"不在输入框里"，那个分支就**永远不会命中**。已加 `Boolean(...)` 收口。这条是新用例直接抓出来的（第一次跑就红），也正是"搬出来才能测"的价值所在。
**同一批还修掉了一处我自己写出来的问题**：照抄 `historyShortcut` 时漏了 `event.altKey` 与"焦点在输入框里"两条判据（对照原文才发现），那会让 Alt+Ctrl+Z 触发撤销、并让用户在数值框里按 Ctrl+Z 撤销**整篇文档**的改动 —— 属于会让人丢工作的那一类。搬动纪律再次生效：**逐字对照原文，不凭记忆重写**。

**这批做对的两件事**：①**依赖传的是取值函数**（`getDocument` / `getExportableDrawings`），不是当次渲染的文档快照 —— 命令面板会在 React 之外调用这些入口，拿快照就会导出"上一次渲染时"的文档（这个项目有前科：确认面板曾提交另一个会话的草稿，根因就是模块级单槽存了快照）；②**给这一族补上了此前为零的测试**（`fileExports.test.ts`，6 例）：下载文件名（空白折成 `-`）、"取调用那一刻的文档"、CAD 工作区之外 DXF / PDF **什么都不发生**（不下载、不报错、也不把那 429 kB 的 `pdf-lib` 拉进来）、保存失败必须说出来（夹具用自引用文档让 `encodeMgeo` 真的抛）。这一族此前**一条测试都没有**，"没人动过所以没坏"是唯一的凭据 —— 拆出来正是为了能直接喂一份文档进去看它交给浏览器什么。
**拆完复核了分包**：入口 **1 630.12 kB**（拆分前 1 629.80，差值是搬动时补的注释），按需 chunk `engineeringExporters` **433.81 kB** 不变 —— 也就是说 `pdf-lib` 仍然是懒加载的。这条必须每次复核，因为 `fileExports.ts` 正好处在那个 import 路径上。

**剩下的大文件**：`App.tsx`（1887 行：文档恢复与自动保存、选择状态、创建流程、CAD 命令、Agent 运行），`threeScene.tsx`（1807 行），`PropertiesBar.tsx` 的面板本体（990 行；按方案继续拆"平面对象 / 模板实体 / 点集多面体 / 测量 / 工程标注"五个面板，并把显示与禁用规则改成"图元类型 → 编辑器"的映射）。`agent-core/src/schemas.ts` 与 Rust `lib.rs` 按方案排在更后。
**关于 `App.tsx` 剩下那部分的实测判断**（本轮勘察，供下一批参考）：它已经不是"还能顺手搬点纯逻辑"的状态了。纯的部分基本搬完了（导出族、id/标签分配、键盘判据），剩下的三类都**真的是有状态的**，各有一处具体的障碍：
①**命令分发**（`case "create-*"` 那一大段）依赖约 20 个闭包 handler，不是纯 reducer —— 要抽就得先给这些 handler 定一个接口；
②**键盘处理**直接闭包读写 7 个 state（`creationStep` / `activeCommand` / `guidance` / `selectedIds` …），抽出去要传 setter 或改成 reducer；
③**恢复与自动保存**（约 140 行）依赖 `persistenceRef` / `restoreSettledRef` / `skipNextDraftSaveRef` 三个 ref 的**时序**，而且里面有两处 e2e 抓出来的守卫（"用户已经动过手就不再覆盖"、"读盘期间切走工作区"）—— 改写的风险明显高于搬动。
所以这三条要么按"先定接口再搬"做，要么按上一批的做法**只搬纯函数**；本轮选择把纯的那部分收干净，而不是硬上。
**`threeScene.tsx` 的实况**（本轮勘察，供下一批参考）：它基本是**一个 1600 行的组件**，内部 `syncContent` 是一个 **770 行的闭包**（447–1213），闭包里直接读组件的 ref。要抽它就得把那些 ref 改成显式传入的上下文对象 —— 那是真改造而不是搬动，所以本轮没有硬上。附带发现：渲染复用读数（`data-scene-created` / `data-scene-reused` / `data-scene-sync-count`）**已经存在**，方案 7 里"Three.js 对象复用 / 销毁数"这一项不需要新建基准。

**方案 4（P2）工作区级代码分包 —— 已完成。** 分包前入口是**单个 2 066.63 kB chunk**（gzip 650.22 kB），构建时那条 "Some chunks are larger than 500 kB" 警告就是它。先量后拆：把 `three` / `pdf-lib` / `react` 临时切成独立 chunk 量出各自体积（529.63 / 428.66 / 221.16 kB），据此判断 `pdf-lib` 是**可以真的按需加载**的那一个（PDF 导出只在用户点「导出」时发生）。做法只有一条纪律：**重依赖不许出现在同步调用链上** —— 三个导出函数改成动态 `import()`，同步要用的 `selectExportableDrawings` 拆到零重依赖的 `persistence/exportableDrawings.ts`。**结果：入口 2 066.63 → 1 629.80 kB（gzip 650.22 → 468.72），首屏少下 436.83 kB / gzip 181.5 kB（−21.1% / −27.9%）**，`pdf-lib` 落进 433.81 kB 的按需 chunk。`three` 仍在入口：立体几何是首屏可达的顶级模块，拆它要连带改 `threeScene.tsx` 的装配方式，留给下一批。

**方案 5（P2）CI 门禁 —— 已完成。** 基线上仓库**没有** `.github/workflows`，所有测试都存在但没有任何东西在每次变更时执行它们。新增 `.github/workflows/ci.yml`，四个作业按成本分层：`checks`（typecheck + lint + Vitest）、`build`、`e2e`（Playwright）、`rust`（`cargo test`，走 `scripts/toolchain.mjs`）。两处刻意的取舍：e2e **不再叠一次构建**（`e2e/global-setup.mjs` 自己会重建当前工作区再 preview，叠了会让 suite 跑在另一份产物上 —— 那份 setup 的注释记着这个坑）；Windows Tauri 打包与大型场景基准不放在每个 PR 上（要签名 / GUI 依赖，且需要稳定机器）。YAML 用真实解析器验过（首版用了 `/* */` 注释，那是**非法 YAML**，已改）。

**方案 6（P2）文档与过期注释收口 —— 已开第一批。** 方案点名的两处**都已确认属实并修掉**：`modelPlanner.ts` 顶部写着"原生工具通道这一轮到不了 / IPC 契约里没有放工具表的位置"，而同一个文件 `:307` 选 `native_tools`、`:344` 把 `PLAN_TOOL_SCHEMA` 真的发了出去，Rust `provider_run` 也收 `tools` —— **注释与代码说的正好相反**；README 第 196 行把 `native_tools` 列进"仍未接"，而同一份文档的 233/241/303 行都写着它已经补上了。另外给 README 加了**唯一一段"现在时"的当前基线**（明确写着"带日期的段落都是历史快照，别当当前值"）与构建体积表。`project-progress.md`（872 KB 的连续日志）按方案拆成"当前待办 / 架构说明 / 归档"属于独立的一批，未做。

**方案 6 第二批：行号引用会悄悄失效（2026 年本轮）。** 方案 6 点名"过期注释会直接误导下一次实现"。本轮抓到一个具体而典型的例子：**我自己在 P0 那批写下的三处引用已经全错了。** `packages/scene-graph/src/actions/index.ts:174`、`actions.test.ts:262`、`apps/web/src/App.tsx:809` 都写着"现场见 `docs/project-progress.md:469`"，而本轮改动把这一节推到了 **526 行** —— `:469` 现在指向的是另一节讲 `canPlan` 的文字。也就是说：**任何一次对进度档案的编辑都会让全仓的"`:行号`"引用静默变成错的**，而错误方向是"把你送到一段讲别的事情的文字上"，比死链更难发现。
**修法不是把 469 改成 526**（下次编辑又会错），而是**改成按小节标题引用**：三处统一写成「`docs/project-progress.md` 的「Agent 造的实体改「朝向」画布不动」一节」，并附上日期与用户原话。标题比行号稳定得多，而且点开文档一搜就到。纪律记在这里：**引用进度档案一律用小节标题，不写行号。**
（顺带确认：全仓再无 `project-progress.md:<数字>` 形式的引用。）

**方案 6 第三批：把"当前"与"归档"分开（2026 年本轮）。** 评审方案 6 的要求是"让 README 只保留产品定位、启动方法与当前稳定能力；另设架构说明、当前待办与版本变更记录；长篇实施日志归档"，并点名"对可以从代码得到的值，尽量用脚本生成或链接到唯一来源 —— 一个量只能有一个数"。
**这一批做的是其中最要紧的一半：当前状态从归档里独立出来。** 新增 [`docs/current-status.md`](current-status.md)（66 行）作为**"现在时"的唯一一处**：门禁读数、各方案做到哪一步、还没做什么、如实缺口。三处配套改动：①本文件顶部加了"这是归档，不是当前状态"的横幅，并把原来那三行"最后更新 / 当前阶段 / 总体状态"**明确标注为归档快照**（它们其实是 2026-09-22 的、5 149 + 1 218 字符的两条超长行，摆在文件最上方最容易被当成当前值 —— 这正是评审说的"历史快照与当前事实混在一起"）；②README 的"当前基线"数字块**删掉、改成指向唯一真源**（原来同一组数字会同时写在 README、当前状态、归档三处，必然漂移 —— 这个项目已经因为"同一个量有两个数"吃过亏：摘要上限 16 000 / 16 000 / 16 384 三个值造成"桌面接受、浏览器拒绝"）；③README 的文档索引改成"当前状态 / 归档"两条并写明各自该看什么。
**为什么没有按字面拆成三个文件**（当前待办 / 架构说明 / 归档）：审计过实际结构之后，`docs/feature-catalog.md`（102 KB，含「当前已实现 / 下一阶段功能 / 明确限制」）**已经承担了"架构与能力"那一半**，`docs/superpowers/{specs,plans}` 承担了设计真源。真正缺的只有"当前"这一份。**再造一个"架构说明"只会变成第四份会漂移的副本** —— 评审的原意是消除副本，不是增加分层。
**顺带确认**：全仓没有任何测试断言这些文档的内容（只有两处注释引用），所以这次重组不影响门禁。

**方案 7 第三批：密集两两相交 + 连续拖动 300 帧（2026 年本轮）。** 补上评审点名里还缺的两条，基准从六条到**八条**：
①`denseIntersections/200x200` —— 200 条过原点的直线 + 200 个派生交点，每次重算都要解析来源并求交（**实测 1.5 ms**；做了**扩展性核查**：把密度提到 16 倍（800×800）读数 6.2 ms，即**线性扩展**，说明这条量的确实是求解成本而不是常数开销）；
②`drag/300-frames` —— 连续 300 帧拖动（每帧 `applyOperation(updatePrimitive)` → 整份文档 `structuredClone` → 重算），**实测约 750 ms，即 ≈2.5 ms/帧**，对照 60 fps 的 16.7 ms/帧预算余量充足。评审点名的场景至此**全部覆盖**，只剩"主线程长任务（`longtask`）"需要在浏览器里量、纯 node 基准拿不到。
**这批两次被自己的夹具绊住，都值得记下来**：
①**拖动的第一帧是空操作**。夹具把那颗点种在原点，第一帧"移到 (0,0)"其实是空操作，`applyOperation` 如实回 `changed: false` —— 我的循环因此抛错。**这是被测代码的正确行为**（空操作不该进撤销栈），错的是夹具；改成从 (1,1) 起步。
②**"交点算出来了"差点被写成一句空话**。断言最初写成"坐标离原点很近"，而夹具里那些坐标**初值就是 0** —— "算出来了"与"根本没算"长得一模一样。改成先把那个交点**种成 (999, 999)** 再重算：只有真的覆盖才会回到原点附近（实测种 999 → 重算后 `x ≈ -1.4e-14`）。这类"断言对着初值成立"的坑在基准里格外危险，因为基准通常只打印读数、断言很松。

**方案 7 的一条自我修正：校准用例自己红过一次（2026 年本轮）。** 上一批加的"基准真的能报警吗"校准用例用的是**单趟**计时做单调性判据（"4 趟必须比 1 趟慢"），结果在整仓并发跑的时候红了：`solidStatusReport` 单趟约 6 ms，一次 GC 尖峰就让 1× 的读数（43.5 ms）**大于** 4×（38.7 ms）。**判据没错，错的是"单趟读数能代表成本"这个假设。** 改成两支各取多趟里**最快**的一趟（噪声只会让某趟变慢、不会让它变快，最小值因此最稳），阈值从 2 倍降到 1.5 倍。改完连跑 4 次基准、3 次全量套件均干净（实测 1× 最优 3.5～6.4 ms、4× 最优 14.1～16.6 ms，分离度 3～5 倍，余量充足）。这条也解释了为什么**不能**给基准写紧阈值：本机同一份代码的读数抖动就有数倍。

**方案 3（P1）几何 Worker —— 客户端已交付；实测表明值得接线（2026 年本轮）。** 评审说缺"宿主侧调用点"，本轮把它建出来了：`apps/web/src/agent/geometryWorkerClient.ts` —— 建 Worker、按 `requestId` 配对响应、核对 `runId`/`draftId`/`draftVersion` 信封、超时、丢弃过期响应、卸载，**9 条用例**逐条钉住。Worker 是**注入**的，所以这些规则在 jsdom 里能直接测；生产侧 `spawnGeometryWorker()` 用评审原文那个写法（`new Worker(new URL("./geometry.worker.ts", import.meta.url), { type: "module" })`）建真 Worker —— 它单独一个函数，因为那串是 bundler 语法、在 node/vitest 里没有意义（规则与接线分开测，与 `workerRuntime.ts` 同一条理由）。
**四条纪律各自对应一个具体的坏结果**：①按 `requestId` 配对 —— 不配就是"A 的响应被当成 B 的结果"，即预览偶尔串版；用例里**故意乱序回**（第二条先回），断言第一条仍在等待，证明它不是"收到就算"；②核对信封 —— 用户继续编辑之后旧版本的编译结果会覆盖新预览，用例断言 `stale_artifact` 并指出是哪个字段不符；③超时 —— 没有它就是界面永久卡在"处理中"而用户看不出原因；④Worker 崩了**如实失败**（`worker_error` + 原始消息），**不静默改走同步路径**（悄悄降级正是这条通道"一直没接线却没人发现"的成因）。
**接线还差的那一步是契约决定，不是工作量**：`CommitterPort.stage` 本来就是 `Promise`、协调器本来就 `await`（零成本），但 **`DraftStorePort.stage` 是模型可见的工具且同步**；`preflight` 同样经它暴露，也不是"另一条路"。所以要么承认那一层异步化（工具派发本来就在 async 上下文里），要么让 Worker 只服务 `CommitterPort` 那一侧。**两个选项都不需要重写管线**，但都改契约，不该顺手做。

**方案 3 的决定依据：编译成本实测，以及一次被读数推翻的判断（2026 年本轮）。** 评审对方案 3 的口径是"应当先量真实耗时"，而**编译这一步从来没量过**。新增 `apps/web/src/agent/compilePlan.bench.test.ts`，用规格 §8 的**两道代表题**。
**实测**：代表题① **4.8 ms**、代表题② **2.2 ms**；同一份计划放在 **2000 图元**的基准文档上 **73 ms**（**线性增长**：做过 0 / 500 / 2000 三档的扩展性核查）；**过线程边界的复制成本（`structuredClone` 同一份文档）只要 1.0 ms**，比值 **76 倍**。
**关键的一次拆分**：把成本拆开量过 —— 2000 图元的 `id` 数组 0.01 ms、`new Map` 0.05 ms、`createIdAllocator` 0.07 ms，**都不是瓶颈**；瓶颈是 `compilePlan` 自己的扫描与校验。这一条决定了 Worker 值不值：成本若在"读文档"，Worker 也要读一遍等于白搬；**成本在"算"，Worker 就能把它挪出主线程**。
**因此上一轮的"暂不接线"是错的，已推翻。** 上一轮我记的是"搬进 Worker 换来省下 4 ms，却要付结构化克隆整份文档的钱，得不偿失" —— 那句话的前提（复制成本可观）**没有量过**，而实测是 1.0 ms，与 73 ms 差两个数量级。这正是"先量再决定"的意义：**一个没量过的假设可以让正确的方案被否掉**，与之前那个记错的阻塞理由（"管线全同步"）是同一类错误，只是方向相反。
**正确的结论**：典型文档上编译只要个位数毫秒（那 73 ms 只在"约 2800 图元"这种真实大文档上出现 —— 评审点名的目标场景正是"约 100 个三维实体"，P0 之后一个立方体 28 个图元）；大文档上 **70 ms 的主线程卡顿值得用 1 ms 的复制换掉**。所以**接线值得做**，工作量是 `DraftStore.stage` 改返回 `Promise` + `DraftStorePort.stage` 标异步（派发层已支持）+ `agentRuntime.ts` 两处接上客户端 —— **不是重写管线**。
顺带把"没有更窄的缝"这件事查实了：把 Worker 只挂在 `CommitterPort` 一侧**不行** —— 两个端口**刻意共用一个草稿存储**（`agentRuntime.test.ts` 有一条用例钉着"tools 与 committer 共用同一个 draft store"），分叉会让模型看到的草稿与提交的不是同一份。`ToolPort.call` 已经是 `Promise`，所以"工具派发支持异步工具"这一点本来就成立。

**更正（本阶段）**：此前几批把这条记成"接进去就要把 `stage()` **及其调用方全部**改成异步 —— 那是 Agent 管线的深度改造"。**那个说法是错的** —— 本轮核对签名时发现 `CommitterPort.stage` 本来就返回 `Promise`、协调器本来就 `await` 它。**一个记错的阻塞理由会让后来人不去做本来做得到的事**，所以这条更正本身也是"如实"要求的一部分。

**方案 7（持续）大型场景性能基准 —— 已完成（八条场景）。** 新增 `packages/scene-graph/src/largeScene.bench.test.ts`（`npm run test:perf`）：
①`addPrimitives/1000-planar` —— 一笔事务落进一份已有 1000 图元的平面文档（`applyOperation` 从整份文档 `structuredClone`，所以这一条量的就是"文档大小对**每一笔编辑**的固定成本"，用户每拖一下都要付）；②`recomputeDerivedObjects/100-solid` —— 100 个带物化拓扑的立方体（约 2800 图元）全量重算，对应"打开文件 / 撤销到某一版"；③`solidStatusReport/100-solid` —— **属性面板每次选中都会跑**的派生读数（外接球 / 内切球 / 截面）；④`encodeMgeo/large` —— 保存与 Agent 提交算内容指纹的序列化成本；⑤`roundTrip/large-mgeo` —— **保存与恢复的往返**（`decodeMgeo` + 全量重算；"打开文件"的另一半）；⑥`solidStatusReport` 的 1× / 4× 校准项。
**本机实测（2026 年本轮）**：**8.1 / 23.5 / 6.5 / 5.7 / 17.2 ms**；校准 1× = 5.9、4× = 21.2 ms（线性，证明计时本身没坏）。
**上限是量出来的、且刻意留了约 100 倍余量**（1 s / 3 s / 0.5 s / 1 s / 5 s）：评审的口径是"基准先记录趋势，再为关键交互设可接受上限"，而写死 `toBeLessThan(200)` 会在别人的机器上变成随机失败 —— 随机失败的门禁最后一定会被关掉，那等于没有门禁。所以它的定位是**报警器**（抓"复杂度写错了"，例如 G1 那次从秒级开始的退化），不是性能目标；想让某个交互更快应该盯 `PERF` 那行读数。CI 的 `checks` 作业会把这几行落进 job summary，PR 页面上直接看得到趋势。
**局部重算那一条给的是等价性判据，不是"局部更快"。** 第一版用"直线链：每段与上一段相切"，实测**局部比全量还慢**（0.9 ms vs 1.6 ms）且两者都随链长线性增长 —— 那条链根本不是真派生链，差一点就把这种没意义的读数当成"基准已就绪"交出去；改成约束链后（0.3～0.6 ms vs 0.7～1.1 ms）**仍然局部更慢**。真实原因是这一档**固定成本压过增量收益**：两条路径都要先 `structuredClone` 整份文档（800 图元），局部还要多付一遍受影响集遍历。**这不是缺陷，是一条成本结构事实**，也是方案 3"要不要搬进 Worker"那个决定的输入之一。所以这条改成钉一条**真性质**：`applyOperation` 走的局部路径必须与全量重算**逐图元一致**（分叉了用户就会看到"改动之后画布与打开文件时不一样"）。
**已覆盖**：`threeScene` 的渲染复用读数（`data-scene-created` / `data-scene-reused` / `data-scene-sync-count`）在本批之前就有了，所以方案 7 里"Three.js 对象复用 / 销毁数"这一项**不需要新建基准**，只需后续把它的读数也接进同一条趋势线。
**未覆盖**：密集两两相交、连续拖动 300 帧、主线程长任务（`longtask`）。


**本批验证证据**（含方案 3 客户端与编译成本实测、方案 1 验收标准与计数核对、方案 2 第二至四批、方案 6 第二至三批、方案 7 三批之后复跑）：单测 **220 文件 / 2672 用例通过 + 1 todo（零失败）**（本轮新增 9 条 worker 客户端用例 + 4 条编译成本用例）；`npm run typecheck` **6 个 workspace exit 0**；`npm run lint` **0 error / 14 warning**（与基线一致）；`npm run build` exit 0，入口 **1 630.12 kB（gzip 468.81 kB）** + 按需 chunk `engineeringExporters` **433.81 kB（gzip 179.75 kB）**（**`pdf-lib` 仍是懒加载**）；`npm run test:perf` 八条场景 + 编译成本四条（**4.5 / 1.4 / 4.0 / 59–69 ms**）；全仓**再无** `project-progress.md:<行号>` 形式的引用；**当前状态已独立成 [`docs/current-status.md`](current-status.md)，README 不再自带基线副本**；所有改动的源文件与文档均通过 UTF-8 严格解码（0 个替换字符，`docs/project-progress.md` 里那 1 个是既有记录里的乱码示例）。`npm run build` exit 0，入口 **1 630.12 kB（gzip 468.81 kB）** + 按需 chunk `engineeringExporters` **433.81 kB（gzip 179.75 kB）**（**`pdf-lib` 仍是懒加载**）；`npm run test:perf` **八条场景**（`addPrimitives` ~6 ms、`100-solid` ~24 ms、`solidStatusReport` ~5 ms、`encodeMgeo` ~4 ms、`roundTrip` ~15–20 ms、`denseIntersections/200x200` ~1.5 ms、`drag/300-frames` ~750 ms ≈2.5 ms/帧、`dag` 等价性 + 校准）；全仓**再无** `project-progress.md:<行号>` 形式的引用；**当前状态已独立成 [`docs/current-status.md`](current-status.md)，README 不再自带基线副本**；所有改动的源文件与文档均通过 UTF-8 严格解码（0 个替换字符，`docs/project-progress.md` 里那 1 个是既有记录里的乱码示例）。**另记一次工具链抖动**：某一次全量跑报 `Errors 1` 而 2641 个用例全绿，紧接着重跑即干净（2642 passed / 0 error）—— 判定为既有的偶发问题（WebGL/jsdom 环境那一片噪声），与本轮改动无关，因为同一份代码单独跑 `packages/scene-graph`（320 例）与 `apps/web`（1253 例 + 1 todo）都零错误。以下为方案 2 第一批（`operations.ts` 拆分）当时的读数：单测 **215 文件 / 2629 用例通过 + 1 todo（零失败）**；`npm run typecheck` **6 个 workspace exit 0**；`npm run lint` **0 error / 14 warning**（与基线一致，拆分引入的 5 条未用 import 已清掉）；`npm run build` exit 0，入口 **1 629.80 kB（gzip 468.72 kB）** + `engineeringExporters` **433.81 kB（gzip 179.75 kB）**；Playwright 冒烟 `app-modules.spec.ts` **2/2**；`operations.ts` 与 `solidGeometry.ts` 均通过 UTF-8 严格解码（**0 个替换字符**）且改动只含预期区间（`git diff --numstat` = 2 insertions / 301 deletions）。


### 发布 v0.2.0（2026-09-23，本机实测）

- **取回最新版**：`git fetch` 看到协作者 `niujin66` 的 24 个提交（`f4d4338..d370698`，80 文件 / +3930 / −289，全部是本轮审查发现的修复 A/B/B续/B再续/B三续/B四续 + 正四面体切片 + 四条现场修复），`git merge --ff-only origin/main` 快进成功；依赖清单无变化（无需 `npm install`）。**本地复核这套最新代码**：`npm test` 215 文件 / **2611 用例通过 + 1 todo**、`npm run typecheck` 6 个 workspace exit 0、`npm run lint` **0 error / 14 warning**、`npm run test:e2e` **140/140**、`npm run test:rust` **232 例通过 + 0 failed + 3 ignored**（16 个目标）。
- **版本号**：`tauri.conf.json` 与 `Cargo.toml` 由 `0.1.0` 提到 `0.2.0`（`Cargo.lock` 随之刷新），提交 `4d9d5bc`。npm workspace 的 `package.json` 版本**刻意不动**：动它会与 `package-lock.json` 失配，而对产出的安装包没有任何影响。
- **构建**：`npm --workspace @draw/desktop run bundle` **exit 0** —— 先把前端重新构建进 `build-check/mathcanvas-current`（`index-Cu1I-5pp.js` 2.06 MB），再编译 release（`Compiling mathcanvas-desktop v0.2.0`，1 分 45 秒），最后由 WiX 3.14 与 NSIS 3.11 产出两个安装包。
- **产物与实测**：免安装 `mathcanvas-desktop.exe` **16.31 MB**、`MathCanvas_0.2.0_x64-setup.exe` **4.70 MB**、`MathCanvas_0.2.0_x64_en-US.msi` **6.43 MB**（SHA-256 前 16 位分别为 `12D13030A8EAB776` / `867E68E165D140CB` / `88CFD59B318ABEA6`）。**免安装 exe 真的启动过**：启动后 10 秒进程仍存活、窗口标题 `MathCanvas`、47 个 WebView2 进程，随后干净退出（`still_running=False`）。**如实**：两个安装包**没有在本机实际安装过**（只是构建成功）。
- **发布**：`git tag -a v0.2.0` → 推送 `main` 与 tag → 用 GitHub API 建 Release（id `394722079`）并上传三份产物，逐份 `HTTP 201`；远端复核 `tag=v0.2.0`、`published=2026-09-23T14:03:15Z`、`draft=false`，三个 asset 的字节数与本地一致。Release 页：<https://github.com/Huo0077/mathcanvas/releases/tag/v0.2.0>。**期间一次网络故障**（node fetch 连 `api.github.com` 被 `ECONNRESET`）已由重试绕过，重试前后状态幂等、未产生重复 Release 或重复 asset。
...
### 平面文档里让 Agent 建立体：模型"就地凑合"用平面点拼立体（2026-09-22，**用户现场**）

- **用户现场**：在**平面几何**文档里要求画立体图形，得到的是**一组平面对象**（模型用 `planar.create_point` 拼了四个"顶点"，再配一笔棱柱）。用户那句追问最准：*"四棱锥是立体几何，怎么生成的是平面的"*。
- **为什么会这样**（三处代码，逐条可查）：
  1. 运行的目标就是**当前文档**：`agentRunner.ts` 的 `target: handleOf(live)`（`live` = 界面上正在编辑的那份）。
  2. 这个工作区**原样进了模型的上下文**：`contextBuilder.ts` 的 `workspace: input.run.target.workspace`，提示词里的 binding 会把它写出来（`systemPrompt.ts`）。也就是说"你在平面几何里发话"⇒ 模型看到的是 `workspace: conics`。
  3. **自动切工作区发生在"计划之后"**：协调器的顺序是 **计划 → `prepare`（切工作区）→ 编译**。所以上一批修好的那一步**只能让计划变得合法，改不了模型已经写下的东西** —— 而 `geometry3d` 的文档同样接受平面动作，于是那四笔 `planar.create_point` 照常编译进去。
- **一句话**：**这份计划是"模型照着它以为的工作区"写的**，而宿主切工作区在它之后。上一批把"必然失败（`workspace_mismatch`）"换成了"如实执行一个坏计划"，这件事才第一次看得见 —— 那次修复本身没错，错在模型**以为**工作区是限制。
- **改法**：在策略文本里明说 ——
  > **工作区不是限制**：绑定里的 `workspace` 只是这份文档现在在哪个工作区，宿主会按你的计划自动切过去。要画立体图形就直接用 `solid.*` 动作，**不要用 `planar.*` 的点去拼立体的顶点** —— 那样得到的是一组平面对象。

  放在 `canPlan` 那一段的**开头**（动作菜单之前）：它是"该怎么选动作"的前置规则，不是场景数据。
- **RED→GREEN**：`systemPrompt.test.ts` **+1**（断言这两句逐字在策略文本里）—— RED 是 `expected '你是 MathCanvas 的构图助手（提示词版本 mathcanvas.…' to contain '工作区不是限制'`。**这条 RED 本身就是变异证据**：把那句话删掉，它必然再红。
- **立刻可用的绕过办法**：先在界面上切到**立体几何**再发那句话 —— 模型拿到的 binding 就是 `geometry3d`，它才会用 `solid.*`。
- **如实缺口**：
  1. 提示词只是一句**规则**，模型可能照旧不遵守；真正的兜底（"计划混合平面与立体时先问一句"）**本批没做**，属独立切片。
  2. **正四面体仍然画不出来**：本项目的棱锥是四棱锥（`baseSize.x/y` + `height`），正四面体是三棱锥，需要新增图元（内核 builder + DSL schema + 渲染 + 属性面板 + 能力清单 + 规划提示词）。
  3. 这句话**只对模型路径生效**：本地确定性规划器不走这段提示词（它认的是写死的指令表）。

**验证（本机实跑，2026-09-22）**：`npm test` **215 文件 / 2603 用例通过 + 1 todo**（起点 2602，+1）；`npm run typecheck` **6 个 workspace exit 0**；`npm run lint` **0 error / 14 warning**（基线）。本批只动 TypeScript（提示词），**Rust 侧未改动、未重跑**。

### 「正四面体」被做成了三棱柱：没有的图元必须如实说（2026-09-22，**用户现场**）

- **用户现场**：按上一批的提示词改法再发一次"画一个正四面体 ABCD，棱长为 3"，得到的实体是**三棱柱**。
- **这不是模型偷懒，是动作词汇表里根本没有那个形状**：
  - `solid.create_template` 只有 `cube` / **`pyramid`（四棱锥：底面 `baseSize.x/y` 是矩形，5 个顶点）** / `cylinder` / `cone`；
  - `solid.create_prism` 是"底面多边形 + 拉伸向量" —— 拿三个点当底再拉伸，得到的就是**三棱柱**（6 顶点、2 个三角面 + 3 个矩形面）；
  - 而正四面体是**三棱锥**（4 顶点、4 个三角面、6 条棱）：**既不是四棱锥，也不是棱柱**；
  - `polyhedron3` 在能力登记表里是 `temporarily_unavailable`（`capabilities.ts:101`：*"no action handler: topology is materialised by the kernel"*）—— 文档模型里有多面体、内核也会物化它（`solid-builders.ts:267`），但**动作层没有任何入口**。
  ⇒ 模型手里没有任何动作能表达它，于是拿最接近的**冒充**了。（同一件事手工也做不到：界面上只有"添加棱锥"，那是四棱锥。）
- **顺带确认**：上一批那条"工作区不是限制"**生效了** —— 这一轮模型确实用了 `solid.*`（不再用 `planar.*` 的点拼顶点），只是形状仍然不对。
- **改法（C）**：在策略文本"该怎么选动作"那一段再加一条 ——
...
- **RED→GREEN**：`systemPrompt.test.ts` **+1**（断言"没有三棱锥""不要拿别的形状冒充"逐字在文本里）—— RED 是 `expected '…' to contain '没有三棱锥'`；这条 RED 同时就是变异证据。
- **如实缺口**：
  1. 这仍然只是**一句规则**：模型可能照旧用棱柱冒充。要真正兜住得在**传输/编译层**禁掉"用别的形状冒充"，而那需要一个"用户要的是什么形状"的判据 —— 属独立切片，本批没做。
  2. **正四面体仍然画不出来**。要按字面成立只有 B1（新增 `solid.create_tetrahedron`，构造现有的 `polyhedron3`）或 B2（新增独立图元）—— 本批没做，等用户定。
...
- **改法（一个构造器 + 一个参数）**：
  1. **内核**：新增 `RegularPyramidInput` / `regularPyramidShape` / `buildRegularPyramid`，注册为构造器 `regularPyramid`。底面 N 个顶点按 `2πk/N` 均匀取（第一个在 +x 方向），第 N+1 个顶点的**水平坐标就是底面中心**、z 高出 `height` —— 于是"顶点在形心正上方"与"底面各边等长"都是**构造保证**的，不是算出来的。
  2. **正四面体收编成特例**：`regularTetrahedronShape` 改为委托 `regularPyramidShape`（`sides = 3`、外接圆半径 `a/√3`、高 `a·√(2/3)`）。用例里有一条**逐字比对两份几何**的断言（`toEqual`）—— 将来谁再把正四面体写成一套独立几何，它就会红。
  3. **动作层**：新增 `solid.create_regular_pyramid`（**底面中心 + 边数 + 外接圆半径 + 高**）；id / 标签助手泛化成 `solidChildIds` / `labelSolidChildren`（顶点 A…、棱 `棱 n`、面 `面 n`），正四面体与正 N 棱锥共用。工作区守卫、输入不合法时不产出任何操作 —— 与另外两个立体动作同一套纪律。
  4. **登记表 / 能力 / 清单 / 提示词**：动作 id +1、`CAPABILITY_FOR_ACTION` +1、`spatial-modeling` 清单 +1 并**更新签名哈希**；`schemas.ts` 新增契约 —— **`sides` 走 `ask_user`**（"底面是正几边形？"：替用户挑一个"正五棱锥"就是替他改题），`radius` / `height` 走 `infer_from_facts`（从原话读数字，读不到取默认值并写进 `assumptions`）。提示词两处同步列上它。
- **守卫如实发威（两处既有用例按设计变红，已同步）**：`actionIds.test.ts` 的写死计数 25 → **26**；`agentRuntime.test.ts` 的 `availableActions` 期望值补上新动作（清单变了，上下文当然跟着变）。
- **验证（本机实跑，2026-09-23）**：内核 `solid-builders.test.ts` **35/35**（含正 N 棱锥的"底面精确 / 顶点在形心正上方 / 边数或尺寸越界必须被拒"三条 + 上面那条同源断言）；动作层 `actions.test.ts` **22/22**（正五棱锥：顶点 6 / 棱 10 / 面 6、标签 A–F、别名指向多面体；越界输入不产出）；全量 `npm test` **215 文件 / 2624 用例通过 + 1 todo**（起点 2619，+5）；`npm run typecheck` **6 个 workspace exit 0**；`npm run lint` **0 error / 14 warning**。提交 `031b448`。
- **仍未做（第 2 层，下一轮）**：**任意多面体** —— 把内核已有的 `fromPoints`（"点集构造多面体"）接到动作层（新动作 `solid.create_polyhedron`：顶点 + 面环），并放开 `capabilities.ts` 里 `polyhedron3` 的 `temporarily_unavailable`。三处已知成本：① 传输层要新加一种字段形状（`faces` 是**二层整数数组**，现在的逐字段校验只有"坐标数组"这一类）；② 放开能力是**产品决定**；③ 顶点标签与 `guidance` 变体。
...
- **取证（没有靠猜，三处真实现场）**：
  1. **跑着的窗口**（CDP 连 `9222`，读 `localStorage`）：`mathcanvas:draft:geometry3d` = `revision: 1`、`primitives: [{"id":"solid-1","type":"cube",…}]` —— 画布上**已经有一个 `solid-1`**（用户此前那条"建一个棱长 3 的立方体"提交的）。
  2. **运行账本**（`%APPDATA%\com.mathcanvas.desktop\projects.db-wal`，文件被运行中的应用锁着，用 `FileShare.ReadWrite` 复制出来再解析）：`run-6-mubf109e` 的全序列是 `1 preflight → 2 observing → 5 planning → 9 compiling（staging **1** action(s)）→ 10 failed（compile_failed: duplicate object id）`。**一次暂存、一个动作**就炸了 —— 排除了"同一草稿反复暂存"这类猜测。`run-9-mubf379f` 是同一条轨迹的第二次复现。
  3. **代码路径**：错误文案 `compile_failed: duplicate object id`（没有 `CODE: message` 前缀）只可能来自 `draftStore.ts` 的 `detail: applied.error`，即 `applyToCandidate → validatePatch` 命中了 `patches.ts:251` 的 `ids.has(primitive.id)`。
- **根因**：`packages/scene-graph/src/actions/index.ts` 的 `createIdAllocator()` 按 `kind` 各记一个计数器，**从 1 开始数**，与目标文档里已有的 id 完全无关。于是"文档里已经有 `solid-1`" ⇒ "新立的第一个立体又拿到 `solid-1`" ⇒ 判重（只按 primitive id 判）当场拒绝。**它在空画布上永远不出现**，所以此前那一轮实跑（0 个对象 → 提交 1 个 → 撤销）恰好全绿，掩盖了这条必然失败。
- **同源的第二份实现**：手工路径 `App.tsx` 的 `nextPrimitiveId(document, prefix)` 一直是 `while (document.primitives.some(p => p.id === \`${prefix}-${index}\`)) index += 1` —— **本来就是对的**（所以用户手工建的立方体是 `solid-1`，第二个手工立方体会是 `solid-2`）。**动作层与手工路径共用同一个编译器的前提下，id 规则不能有两份**，这一批把它对齐。
- **改法**：`createIdAllocator(taken: Iterable<string> = [])` 收**占用集**，`allocate` 跳过被占用的空位（允许稀疏占用：`point-1`、`point-3` 都占着时下一个是 `point-2`），并把已发出的 id 也记进占用集；别名幂等（同一 `(kind, alias)` 仍是同一个 id）**原样保留**。三处调用方把目标文档的 primitive id 传进去：`draftStore.create`（草稿候选的基础文档）、`workerRuntime` 的 `geometry.compile`、`App.tsx` 的手工批量删除路径。**没有引入第二个真相** —— 占用集就是 `patches.ts` 判重用的那份（`document.primitives.map(p => p.id)`）。
- **RED→GREEN（先看红再改）**：`packages/scene-graph/src/actions/idAllocator.test.ts`（新建，3 例：跳过已占用 / 保留别名幂等 / 稀疏占用下不发重号）—— RED 是 `expected 'point-1' to be 'point-2'`；`apps/web/src/agent/draftStore.test.ts` +1 例（基础文档已含 `solid-1` 时暂存立方体必须成功且 ids 为 `["solid-1","solid-2"]`）—— RED 是 `expected false to be true`；`apps/web/src/agent/workerRuntime.test.ts` +1 例（基准含 `point-1`）—— RED 是 `expected 'geometry.error' to be 'geometry.compile.result'`；`apps/web/src/agent/agentRuntime.test.ts` +1 例（**组装好的运行时**：非空文档 → 真实协调器 → 真实适配器 → 真实草稿，落在 `awaiting_confirmation` 且候选 ids 为 `["solid-1","solid-2"]`，真文档一个字节没变）。
- **浏览器级回归**：`e2e/agent-flow.spec.ts` +1 例（"先提交一个立方体，再让同一个规划器建第二个"）。关键断言是"**第二份草稿能出现在确认面板**"—— 撞 id 时草稿根本进不到那一步（旧代码下运行状态直接是 `没有完成`）。用的还是本地确定性规划器（`alias` 恒为 `cube`），所以它复现的正是"同类新建"。
- **如实缺口**：**同一次运行里对同一 alias 重复暂存**仍会以 `duplicate object id` 失败（分配器按 alias 幂等 ⇒ 同一个 id 被二次 `addPrimitive`）。当前生产路径到不了那里（`agentRunner` 每轮新建运行时，适配器的草稿是**每次运行一份**，协调器一轮只 `stage` 一次；`awaiting_confirmation → compiling` 这条边只有调用方再 `start` 一次才会走），所以这一批**只记录不改** —— 真要放行需要先定语义（"同一 alias 的第二次暂存"是**重试**（应当 no-op）还是**改主意**（应当新对象）），而定错了会静默吞掉用户的修改。
...
- **定位（探针，不猜）**：写了一个临时用例，把"Agent 提交一个立方体"的那份文档交给 `migrateLegacySolids`（**应用启动恢复草稿时就会跑它**，`App.tsx` 打开文件 / 恢复文档 / 恢复草稿三处），再走一遍规范化哈希。结果：迁移之后的文档里 `style: undefined`、`label: undefined` **成片出现**（立方体 27 个子对象几乎人人一个 `style: undefined`；圆柱 428 处），哈希当场抛错。
- **根因是"两份实现在说两份话"**：文档的**每一处**落盘与比对路径都是 JSON 语义 —— `JSON.stringify` 直接丢 `undefined` 键、`contentFingerprint`（CAS 的比较基准）也是 JSON 比语义、`migrateLegacySolids` 还**有意**用 `label: undefined` 表达"这个对象没有标签"。只有 `canonicalContentHash`（Agent 预览哈希）把 `undefined` 当垃圾抛出去。空画布跑得通，只是因为空文档里没有子对象 —— 也就是说：**只要画布上有一个立体，Agent 就必然失败**，而错误信息里只有一句内部函数名。
- **改法（两处，各修一半）**：
  1. `packages/agent-core/src/schemas.ts`：`canonicalize` 与 JSON 对齐（`undefined` 等于"没有这个字段"，数组里的 `undefined` 记作 `null`），并把**路径**带进错误信息（`non-finite number at primitives[0].x` / `unsupported value of type function at primitives[0].broken`）—— 判据写死为"哈希值等于同一份数据 JSON 往返之后的哈希值"。真正无法表达的值（函数 / symbol / 非有限数）**照旧拒绝**。
  2. `apps/web/src/solidTemplates.ts`：`realignTemplateChildren` 的"去掉标签"从 `label: undefined` 改成**删掉这个键**（`withoutLabel`）。产生数据的地方也不许写出这种键 —— 只改哈希一侧等于把问题留给下一个人。
- **RED→GREEN**：`packages/agent-core/src/schemas.test.ts` +3（`undefined` 字段 = JSON 往返同值 / 数组里的 `undefined` / 无法表达的值仍拒且**报出路径**）—— RED 是 `expected '…' to be '…'` 与错误信息里没有路径；`apps/web/src/solidTemplates.test.ts` +1（隐藏子对象**不含 `label` 键**，且迁移后的文档可哈希）—— RED 是 `expected true to be false`；`apps/web/src/agent/draftStore.test.ts` +1（**恢复/迁移之后的文档必须还能起草**，就是用户现场那条路径）—— RED 是 `canonicalContentHash: unsupported value of type undefined`。
- **顺带修掉一处类型收窄**：`AgentRuntime.committer` 原先声明成 `CommitterPort`，把适配器自己的 `draftIdFor()`（"这一轮用的是哪个草稿"）从类型上藏掉了；改成 `CommitterAdapter`。这是 `typecheck` 抓出来的（我新写的用例在读这个能力）。
- **真窗口实测（重建 exe → 重启 → 真 DeepSeek）**：
  - `再建一个棱长 2 的立方体` → **等待你确认** · 会新增 1 个对象（共 29 个）→ 确认并提交 → 状态栏「对象 **29**」、对象列表出现「立方体(棱长2)」；
  - 用户原来那条 `已知直四棱柱 ABCD-A1B1C1D1 的底面是菱形，AA1=4, AB=2, BAD=60°, E、M、N 分别是 BC、BB1、A1D 的中点。` → **等待你确认** · 会新增 1 个对象（共 30 个）· 3 条假设（菱形中心在原点 / 底面取 z=0 / 侧棱长 AA1=4）· **0 个页面错误**。这一条**刻意没有点确认**（留给用户过目），但它已经证明"草稿能成型"——此前它必然死在 `duplicate object id` 或哈希上。
- **浏览器级回归**：`e2e/agent-flow.spec.ts` +1 例（**先提交一个立方体 → 刷新页面**（= 重启 + 恢复 + `migrateLegacySolids`）→ 再发一条同类请求，草稿必须照样出现）。第一版把断言写成"共 2 个"，实测拿到的是"共 **29** 个"—— 因为恢复时迁移把立方体的 27 个子对象物化了。这不是缺陷，是那条断言本身写错了：现在断言"总数 > 1 且会新增 1 个对象"（把"迁移有没有跑"变成弱断言，而不是把一个会随迁移漂移的数字钉死）。
- **如实缺口**：内核 `buildSolidTemplate` 仍会给子对象写 `style: primitive.style`（源实体没有样式时就是 `undefined`）。这一批**只让哈希容忍它、没有去改内核**：改它要动 3D 场景里所有读 `style` 的地方，风险与收益不成比例；而"内存文档带 `undefined` 是可接受的"这条现在已经由哈希与 JSON 语义对齐兜住了。`label: undefined` 那条（迁移里**有意**写的）已按"删键"改掉。

### G1 第八批：回环代理**真服务器**落地（Task 1.5 收口）（2026-09-21）

- **交付**：`apps/desktop/src-tauri/src/proxy/server.rs` + `tests/proxy_server.rs`（**11 例**）+ 两个 IPC 命令（`proxy_session` / `proxy_cancel`）+ 常驻 tokio 运行时挂进托管状态。
- **这一批把 G1 Gate 第 4 条从"一半"推到"满足"**：判据层（16 例）之前就有，但**没有真服务器** —— 现在绑定了、路由了、每个请求都过判据了。
- **测试是"真的起一个 socket"**（`tests/proxy_server.rs`），因为有三件只有真服务器才成立的事**在纯函数里看不出来**：①**只绑回环**（绑 `0.0.0.0` 的版本在纯函数里完全看不出来，而那会把持有密钥的代理暴露给同网段）；②**端口由系统给且每次不同**；③**判据真的挂在每个请求上**（不是"开机检查一次"）。
- **手写 HTTP 而不用客户端库**：因为要**伪造 `Host` 与 `Origin`** —— 客户端库会"帮我们"把 Host 设成连接目标，于是"Host 不对必须被拒"这条用例根本写不出来。手写几行请求字节就够了，而且**不给测试树再加一个依赖**。
- **11 例覆盖**：只绑回环 + 端口每次不同 + 会话令牌每次不同 / `baseUrl` 不含令牌 / 无令牌拒 / 令牌错拒 / 外来 Origin 拒（含 `*`）/ **Host 不对拒（即使令牌与 Origin 都对）** / 合法请求过 / **`model` 与 `events` 如实回 501** / 取消置位且可见 / 未知路由 404 / 关掉之后端口不再接受连接。
- **一条刻意的断言**：`the_model_and_event_routes_report_that_forwarding_is_not_wired_yet` —— 这两条路由**如实回 `501 not_implemented`**，而不是回一个看起来像成功的东西（200 + 空事件会让前端拿着空结果继续推理）。**这条断言也是"下一步"的哨兵**：哪天有人把转发接上了，它会红，那一刻正是"该把前端从'未接通'切到'已接通'"的有意决定。
- **三个依赖各有一个理由**（写在 `Cargo.toml` 里）：`tokio`（要同时持有入站请求与出站流两条生命周期、还得能取消，同步模型做不到）；`axum`（固定路由需要它落，"没有任意 URL"那条约束才有地方）；`reqwest` + **`rustls-tls`**（刻意**不用**系统 TLS —— "云端必须 HTTPS 且要验证证书"这条判据不该随用户机器的 TLS 配置变化）。`rand` 显式声明而不是自己写随机数：令牌的不可猜测性是这条边界的地基，而"为什么这个随机数够随机"不该由我来论证。
- **一条极难查的故障被提前挡住**：`tokio::runtime::Runtime` 一被 drop，跑在它上面的服务器任务就停了 —— 而"服务器莫名其妙不响应了"这种故障几乎没有线索（应用没崩、日志没报错、只是连不上）。所以运行时被 `app.manage(ProxyRuntime { … })` **持有住**，生命周期跟着应用走，而不是靠"记得别 drop 它"。

**验证证据（本批）**：Rust **99 例通过 + 1 例 `#[ignore]`**（单元 8 + project_repository 16 + provider_profiles 14 + providers 17 + proxy 16 + **proxy_server 11** + secrets 8 + shell_smoke 9）、单测 188 文件 / 2113 用例全通过（零跳过）、typecheck exit 0、lint 0 error / 14 warning（基线）。

### G1 阶段状态（截至 2026-09-21，**唯一真源**；计划文件里的复选框**不作为**进度依据）
...
| 1.3 provider 配置与设置存储 | ✅ | 无 —— 「使用中」的那一份**已被规划器消费**（见「G2 接线」那一批） |
| 1.4 provider 适配器与事件归一 | ✅ | 能力探针的**请求形状**照公开文档拼、**没有对真实服务跑过**（见「G1 第十批」的一处保留）；`native_tools` 通道到不了（`provider_run` 契约里没有放工具表的位置，见「G2 接线」） |
...
| 1.6 SQLite 仓储 / CAS / 崩溃恢复 | ✅ | **界面入口已接**（「项目包…」：导出/导入 + 附件 + 孤儿回收，见「G1 第十四批」）；仍缺一条"某份快照引用了哪些附件"的列举命令，所以界面上的附件列表只覆盖本次会话 |
...
### 项目包：附件列举命令补上（"只覆盖本次会话"那条限制关掉）（2026-09-21）

- **交付**：Rust 一条命令 `read_document_attachments(project_id, document_id, generation)` + 前端 `readDocumentAttachments` + 面板打开时刷新引用列表（**+1 例**，并把这处更正写进用例）。
- **为什么这条命令非有不可**：引用记在**快照**上而不是 head 上（撤销回旧版本时那一版的图必须还在），所以"这份文档有哪些附件"这个问题**没有唯一答案** —— 有意义的问法是"**这一版**引用了什么"。在此之前界面只能列出本次会话里附加过的那几个，重开应用就数不出来了。仓储侧的 `attachments_of` 与它的测试**早就存在**（`repository_package` 用过），缺的一直是**命令与界面接线** —— 这也是本轮那处更正的落点：我此前把"缺命令"记成了"缺能力"，说得太重。
- **列表是两处的并集**：**这一版快照引用的**（从库里读回来，重开应用仍在）∪ **本次会话附加的**（带本地文件名与大小 —— 名字是本地文件的事，库里只记哈希与大小）。导出带的就是这个并集。
- **刚附加就刷新**：`put_attachment` 的两阶段写里已经记了引用，所以附加成功后立刻重读一次 —— 否则界面要等到下次附加才显示它。
- **一处如实显示**：浏览器里列举同样失败（没有项目库），此时显示"**这一版快照还没有引用任何附件**"，而不是伪造一份列表；空态文案也据此改了（e2e 已同步）。

**验证证据（本批）**：Rust **193 例 + 1 ignored**（命令哨兵按设计更新过一次）、单测 **192 文件 / 2192 用例**、typecheck exit 0、lint 0 error / 14 warning、**e2e 122/122**。
...
- **装了两次，因为第一次不够**：`winget install Rustlang.Rustup` 装好 rustup 1.29.1（`cargo` / `rustc` 1.98.1，`stable-x86_64-pc-windows-msvc`），但**一次真实链接就暴露了缺口**：`rustc main.rs` 报 `error: linker 'link.exe' not found` —— 本机有 Windows SDK（`KitsRoot10` 指向 `C:\Program Files (x86)\Windows Kits\10\`、`kernel32.lib` 在），却**没有 MSVC C++ 工具集**（`vswhere` 不存在、`VC\Tools\MSVC` 不存在）。补装 `Microsoft.VisualStudio.2022.BuildTools`（组件 `Microsoft.VisualStudio.Component.VC.Tools.x86.x64` + `Windows11SDK.26100`）之后，`link.exe` 14.44.35207 就位，同一条链接测试通过并打印 `link-ok`。
- **判据是"真的链一次"，不是"命令装完了"**：`winget` 报"已成功安装"时 `cargo --version` 也会成功 —— 而 `rustc` 能编译、链接却失败的组合是**存在**的（就是中间那一步的状态）。所以这里的证据是 `rustc main.rs -o link-test.exe` → exit 0 → 运行输出 `link-ok`。
- **复核（都是本机实测）**：`npm run test:rust` **179 例通过 + 1 例 `#[ignore]`**（与档案记录逐项一致：单元 7 + project_repository 19 + provider_adapter 16 + provider_capability 18 + provider_profiles 22 + providers 19 + proxy 16 + proxy_server 16 + repository_package 29 + secrets 8 + shell_smoke 9）；`cargo clippy --all-targets` **exit 0 / 0 条警告**；`npm run build` exit 0，产出 `apps/desktop/src-tauri/target/release/mathcanvas-desktop.exe`（15.9 MB）。
- **并且真的启动了那个 exe**（G1 Gate ① 的那条判据）：窗口标题 `MathCanvas`，12 秒与 25 秒后进程仍在运行、内存 32 MB，截图里能看到窗口内渲染出的完整工作台（文件/平面几何/立体几何/工程制图 + Ribbon + 模块栏 + 代数区 + 画布网格）——也就是说 WebView2 加载成功、`frontendDist` 里那份构建产物是好的。随后 `Stop-Process` 清理，**无残留**。
- **一处如实提醒**：装完之后**已经开着的进程仍然看不到 `cargo`**（winget 写的是用户级 PATH；`npx tauri info` 在旧进程里会报 rustc/cargo 未安装，而同一台机器上新开的终端一切正常）。`scripts/toolchain.mjs` 就是为这件事写的，`npm run test:rust` 与桌面构建都走它。
- **仍然没做的**：`tauri dev` 热重载路径未实测；打包安装器（带 bundle）属 Task 5.4；**没有对真实 provider 跑过一轮**（本机没有配过任何 profile 与密钥）；Gate ② 的密钥手动重启验证需要用户本机操作。

### G2 接线：`PlannerPort` 接到真实 provider，「使用中」的那一份被真的消费（2026-09-21）

- **交付**：`apps/web/src/agent/modelPlanner.ts`（新建，**23 例**）+ `agentRunner.ts` 的 `selectPlanner`（**+6 例**）+ `agentRuntime.ts` 的 `questions()`。这一批关掉的是 G1 第十三批留下的那条如实缺口——"`使用中` 还没被规划器消费，把 `PlannerPort` 接到真模型属 G2"。
- **「使用中」是**规划器自己现取的**，调用方一个 `profileId` 都不传。三道检查各有理由且顺序不能反：①有没有桌面外壳（浏览器里连凭据库都没有）；②有没有选过（`null` 是"还没选"，不是"选了第一份"）；③**这一份有没有密钥**（`secretRef` 缺省时拿它去发请求，报错会指向凭据库，而不是"你还没填密钥"）。解析结果被**钉住**（`resolveProvider: async () => resolution`）：一次运行里模型可能被问两次（含那次修复），两次必须用同一份配置与同一批能力证据，否则第二次尝试其实换了题目。
- **通道由能力证据决定，不由品牌名决定**：`planModelRequest` + `isCapabilityVerified`。`json` 已验证 → **严格 JSON 通道**（整段回复必须就是 JSON）；否则文本通道（允许恰好一层 ```json 围栏）。通道建议按**同一个 `channel` 值**写进提示词——提示词与解析器各判断一次必然分叉，而发错通道的表现是"明明能用却一直解析失败"。
- **失败与"答得不像话"是两种东西**：provider 失败（401/429/连不上）**抛** `ModelPlannerError`（带 `code` + `retryable`），协调器据此落到 `failed` 并把分类带出来；解析失败则**把模型自己说的那个值原样交出去**（能 JSON 解析就是那个对象，不能就是那段原文），让协调器用它手里的 `MAX_PLAN_ATTEMPTS` 与 `repair` 提示再问一次。规划器自己重试会把预算绕过去，而这正是"一次性可见修复"那条判据的落点。
- **取消真的传到 Rust**：`abort` 监听器调 `provider_cancel(runId)`。只把事件过滤掉不够——那次 HTTP 会继续跑到超时，而用户看到的是"按了停止没反应"。
- **收到工具调用就拒绝**：这一轮**没有**发任何工具表，所以任何工具调用都不可能是我们请求的。静默忽略它等于把"模型以为它调用了什么"变成"什么都没发生"。
- **`PlanRequest.model.tools` 第一次真的被用上**：`plan.set_plan` 在表里才允许返回 `kind:"plan"`（模型只能做这一阶段允许它做的事）。在这之前那个字段是"声明了没用"的那一类。
- **`native_tools` 通道这一轮到不了（如实）**：`provider_run` 的入参只有 `messages`，**IPC 契约里没有放工具表的位置**，所以走的是"动作菜单写进提示词 + 解析 JSON 信封"。那个分支是**如实拒绝**而不是静默降级（降级会让人以为工具通道通了）。要让它可用，得先给 `provider_run` 加一个 `tools` 参数（Rust 侧改动）。
- **模型路径下请求全部技能清单（如实取舍）**：本地规划器能精确说出"我这条指令要用哪份清单"，而模型路径**不可能在发请求之前**知道用户想要什么——这一趟请求本身就是为了问它。所以给它整张菜单，靠传输层 schema、动作编译器的语义校验与"写入必须经用户确认"三层兜底。按关键词猜看起来更省，但猜错的代价是某些任务**永远做不了**，而那看起来像"模型不会做"。
- **顺带修掉一句**假话**：`waiting` 分支原先写死"当前没有接入模型服务，本地规划器只认识几条固定指令"。接上模型之后那句话是错的（模型明明问了"半径是多少"）。现在由运行时的 `questions()` 把**规划器真正问的那句**显示出来（与假设同一处产生、同一条理由）。
- **重试走的是策略表，而且真的扣共享预算**（Task 2.3 Step 4：`RecoveryController.decide`）：`transport` / 429 / 5xx **重试同一请求**，认证 / 权限 / 未归类**一次都不多发**，流损坏（`malformed_output`）**不自己重发**——那需要一份**新上下文**，而上下文不是这一层建的（它只拿到协调器组装好的那一份），所以如实交回调用方（界面上的「重试」就是那条路）。两类失败分类（`modelEvents` 讲"provider 说了什么"、`recovery` 讲"该怎么处置"）之间的映射**只有一处**，所以"哪些能重试"仍然只有一个答案。**重试要从 `PlanRequest.budget` 里扣**（`network` + `generation`，先问 `remaining` 再扣）——不扣就等于把协调器的预算绕过去，而计划原文那句 "retry/fallback/repair share one budget" 正落在这里。上限 3 次（含第一次），因为每一次往返都是用户的钱与等待时间。命令的 `Err` 与事件流里的 `failed` **走同一条恢复路径**：两种到达方式对调用方是同一件事，给它们两套处置必然分叉。
- **端到端证据**（用注入的 `runModel` 替身，其余部件全是真的）：模型回一段 JSON → 真实编译 → 隔离草稿 → 确认前文档一个字节不动 → 确认 → 落盘 → **一步撤销**；散文答一次 → 修复通道真的被用上（第二次请求里带着字段路径，且**不回显**模型原话）；provider 失败 → 带分类的失败且文档未动；429 两次后第三次成功 → 真的重试了两次并交回计划；认证失败 → **只发一次**；预算只够一次重试 → 就只发两次（预算更紧则一次都不多）；没有选中时回落本地规划器且**一次模型调用都没发生**。

**验证证据（本批）**：单测 **191 文件 / 2182 用例全通过（零跳过）**（+3 文件 / +48 例）、typecheck 6 个 workspace 全过（exit 0）、lint **0 error / 14 warning**（基线）、**e2e 122/122**。**Rust 侧复跑（同日，工具链装好之后）**：`npm run test:rust` **179 例通过 + 1 例 `#[ignore]`**、`cargo clippy --all-targets` **exit 0 / 零警告**、`npm run build` exit 0 并产出 release exe，**并真的启动了它**（窗口标题 `MathCanvas`、12 秒与 25 秒后仍在运行）。**Rust 源码本身本轮未改动** —— 复跑是为了把"本机没有工具链、那些数字只是历史记录"这条如实缺口关掉。

### G1 第十四批：项目包界面入口——附件 + `.mcanvas` 导出/导入（2026-09-21）

- **交付**：`apps/web/src/services/projectPackageClient.ts`（**11 例**）+ `apps/web/src/components/ProjectPackagePanel.tsx`（**6 例**）+ `e2e/project-package.spec.ts`（**3 例**）+ `packages/agent-core` 新增 `sha256HexBytes`（**+3 例**）+ `WorkspaceTabs` / `AppChrome` 的「项目包…」入口（与"打开/保存 `.mgeo`"同一组，文件级动作只在一处）。
- **这一批关掉的是 1.6 那条"缺界面入口"**：附件两阶段写、`.mcanvas` 打包、孤儿回收在 Rust 侧有 29 + 19 例判据，而**用户在界面上做不到任何一件**。
- **附件哈希必须按原始字节算**（新增 `sha256HexBytes`）：`put_attachment` 会拿调用方声明的哈希校验它落盘的字节，所以哈希**不是**调用方随便给的参数，而是由前端从字节算出来。算的是**原始字节**而不是 UTF-8 编码后的文本——一张 PNG 开头的 `0x89 0x50` 走 UTF-8 会变成四个字节，于是**同一份附件在前端与 Rust 侧算出两个哈希**，症状是"每一次附加都失败，理由却是内容哈希不符"。两条入口（字符串 / 字节）共用同一份实现，新增的用例把它钉住：FIPS 180-4 的公开向量（空串 / `abc`）+ "字节 vs UTF-8 重编码"必须是两个哈希。
- **base64 手写而不用 `btoa`**：后者只吃 latin1 字符串，二进制附件走它必然被改写。二十行换一个不可能出错的转换。
- **三处如实说明摆在界面上**：①**没有原生文件选择框**（能力清单刻意只有 `core:default`，没有 dialog 插件），所以导出目的地与导入路径是**填出来的**（填过一次记住），而不是让一个不存在的选择框假装存在；②**附件列表只覆盖本次会话**——Rust 侧还没有"某份快照引用了哪些附件"的列举命令（`read_document_head` 只回文档本体），所以重开应用之后界面数不出历史附件；③**导入换一世**（每次新的 epoch，于是上一次编辑可能还在途的自动保存立刻 CAS 失败）。
- **不拿假的 generation 去写**：文档还没在项目库里落过盘时，附件会被如实拒绝（"附件挂在快照上，没有快照就没有可挂的地方"），而不是用 `0` 去写一份挂在**不存在的快照**上的附件。
- **导入之后正文要读回来**：`import_package` 只回包的结构（文档 id 列表、附件数、缺失来源），正文得从仓储读 head 再交给宿主——而宿主走的是与"打开 `.mgeo`"**同一条路**（`load`：换文档 + 换一世），所以没有第二套逻辑。
- **一处靠截图发现的静默失效**（值得记）：面板第一版写了 `--color-surface` / `--shadow-soft` / `--font-size-lg` 三个**根本不存在**的令牌（真名是 `--color-panel` / `--shadow-panel` / `--font-ui-lg`）。CSS 变量写错**不会报错**，表现是背景静默变透明——面板浮在画布上没有底。是量了 `getComputedStyle` 的 `backgroundColor` 才确定（`rgba(0,0,0,0)`），改完再看截图。**这类缺陷只有截图或计算样式能发现，测试与 lint 都不管。**
- **一处关于 RED 的如实说明**：这一批**先写用例再写实现**（仓库约定），但"看见红"这一步在新建模块上的形态与改既有代码不同 —— 客户端与面板的第一次运行只报**一个**失败，而那个失败是**用例自己的替身写错了**（替身按调用顺序回答，导致"找不到 = null"那条拿不到 null），不是实现错了。所以这一批的证据是"改完替身之后 11/11 与 6/6"，而真正的 RED 是"模块/组件还不存在"那一步（与 `modelPlanner` 那一批同形）。
- **界面上的 markdown 记号也一并去掉**：提示文字里原样写着 `**永远没有密钥**` 与反引号，用户看到的就是星号和反引号。

**验证证据（本批）**：单测 **191 文件 / 2182 用例全通过（零跳过）**、typecheck exit 0、lint **0 error / 14 warning**、**e2e 122/122**（新增 3 条：入口可达 / 浏览器里如实说"需要桌面版" / 不假装附件已存）、实拍截图一张（面板三个小节 + 令牌生效）。
...
- **交付**：`apps/desktop/src-tauri/src/providers/capability.rs`（`Probe` + 判据 + `check`，**18 例**）+ `provider_check` IPC 命令 + 前端 `checkProviderCapabilities` 与设置界面上的**探测按钮**（`providerProfileClient` 16 例 / `ProviderSettings` 17 例）+ 请求拼装扩展（工具表 / `tool_choice` / 图片）。
- **这一批解决的是"证据的边界"，不是"把请求发出去"**：发请求上一批就做完了。难的是**一次成功的请求到底证明了什么**。把这件事做错的方式非常自然——"请求成功了那就都打勾吧"——而它的后果是界面显示四个绿色徽章，直到真的给一个不支持图片的模型发图，用户得到一句莫名其妙的错误。所以四条探针**各发各的请求**，逐条下结论：

  | 观测到的 | 结论 | 为什么 |
  | --- | --- | --- |
  | 回了文本、流正常结束 | `streaming` = verified | 这正是流式文本要证明的东西 |
  | 只有文本、没有任何工具调用 | `tools` = **unknown** | 请求成功了所以不是 failed；但"模型不肯配合"太常见，不能读成"不支持" |
  | 工具调用里出现**我们的工具名** | `tools` = verified | 它只有拿到工具表才可能说出这个名字 |
  | 图片请求被 400 拒 | `vision` = **failed** | 图片请求的**形状是我们拼的** —— 被拒是我们发错了 |
  | 401 / 403 / 429 / 连不上 / 没有密钥 | 全部 **unknown** | 认证失败对"支不支持某能力"什么都不说明 |

- **最要紧的一条区分："这家不支持"与"我们发错了"必须分开**，因为下一步动作完全不同（前者改配置、后者改代码）。所以图片与工具探针的 400/422 记 `failed`，而认证/连接/放弃配合一律留 `unknown`。用例逐条盯着这条线（`a_refusal_to_use_the_tool_is_unknown_not_failed`、`a_provider_that_knows_the_tool_but_uses_it_wrongly_still_counts_as_supporting_tools`）。
- **一次探测会花掉四发真请求**（文本、JSON、一张 1×1 的 PNG、一次带工具表的请求）—— 所以它是**用户按下去才发生**的，按钮上写明了代价，没有密钥时按钮禁用。打开设置就自动探测会在用户不知情的时候花掉他的额度。
- **工具探针必须强制**（`tool_choice`）：不强制的话模型会跟你聊天，于是"没有工具调用"这个观测毫无意义 —— 而它会被读成"不支持工具"。三种方言的取值不同（OpenAI 兼容与 Ollama 用 `required`、Anthropic 用 `any`），写错那一个的后果是 400，而 400 **正是探针最容易误报的地方**，所以有一条用例专门盯它。
- **图片探针发一张真的 1×1 PNG**，且三家形状各不同（OpenAI 兼容用 content parts 里的 data URL、Anthropic 用 base64 的 `source` block、Ollama 用消息上的裸 base64 数组）。消息里存的是**裸 base64**，`data:` 前缀是拼请求时按方言加的 —— 这一层也被用例钉住。
- **两处如实的保留**：①探针的请求形状照三家**公开文档**拼（与 fixtures 同源），**没有对真实服务跑过** —— 形状错了的表现是"明明支持却被记成 failed"，所以 `vision` 的 400 判据特意记 `failed`（那是要我们改代码的），而任何**说不清**的情况一律留 `unknown`；②`probe_tool_schema` 里的工具描述是英文散文，模型对它的理解程度会影响"肯不肯调"，而这一项没法离线验证。
- **两个替身设计上的假阳性被改掉**（都是这一批自己踩的）：①第一版替身"按顺序回脚本"，于是用例断言的是"第几发请求" —— 拆一次顺序就红一片，而那与证据判据无关；②它没法表达"带图的请求会失败、纯文本的会成功"，于是**根本测不出图片这件事**。现在替身按**请求形状**回答（这一发带图吗？带工具表吗？），"文本 ping 验证不了 vision"这条性质才真的被钉住。
- **测试里还抓到一个我自己写的越界**：`mark_health` 要 `&mut`，我第一版想用 `&self` 加一个裸指针转换绕过去 —— 那是为了省一个 `mut` 而引入未定义行为的风险。改成 `HealthSink::record(&mut self, ..)`。
...
- P4 / P5：**由用户明确排除**（Agent 与 Provider Router、题图解析），不在当前范围（**2026-09-21 更正**：用户随后要求恢复 P4，**P4 已交付** —— 见「G1 阶段状态」表与「G2 接线」；P5 仍排除）
...
P7-1 至 P7-6 与工程工作台层次化改造 Task 1-7 均已完成；P4 Agent、P5 题图解析保持在排除范围内（**2026-09-21 更正：P4 已按用户要求交付**，见「G1 阶段状态」与「G2 接线」；P5 仍排除）。CAD 工作台的后续可选方向（手动拖动视口边界、B-rep/DWG 导入、自动尺寸布局）仍属于设计文档中的明确限制。
...
- **2026-09-22/23 这批（外部审查 32 条 + 真实运行里抓到的四条缺陷 + 正四面体切片）推送记录**：本地 `main` 从 `f4d4338` 走到 `72854ef`，共 **23 个提交**。**推送前**按本文件上面的规矩先 `git fetch origin` 核对 divergence：**ahead=23 / behind=0**（另一会话未并发改动 `main`），可以快进推送。这批内容：`docs`（审计发现登记）→ **修复轮 A**（2 Critical + 14 Important）→ **修复轮 B 五批**（Minor 16 条中 15 条修复 + 1 条如实降级为"尚未接线"）→ **三条真实运行里抓到的缺陷**（密钥用例会删用户的真实凭据、`invalid_type@envelope` 的三处修法、工作区判据看第一笔动作）→ **两条行为纠正**（提示词"没有的图元要如实说"、"工作区不是限制"）→ **正四面体切片**（内核构造器 + `solid.create_tetrahedron` + 登记表/能力/清单/提示词 + 本地规划器）。每条提交都写清 RED→GREEN 与变异证据，或**如实标注**没做的部分。
- **这批的门禁（本机实跑，2026-09-23 收口）**：`npm test` 215 文件 / **2611 用例通过 + 1 todo**、`typecheck` 6 个 workspace exit 0、`lint` 0 error / 14 warning、`build` exit 0、Rust **232 例 + 3 ignored**、`clippy -D warnings` exit 0。**Playwright/e2e 这批没跑**：窗口级验收留给用户，另有"dev 窗口可能拿着半途的模块"这条环境坑（见上文同名一节）。
- **2026-09-23 反向同步（拉取协作者的发布批次）**：`git fetch origin` 看到 `origin/main` 前进 2 个提交 —— `4d9d5bc`（`chore(release): MathCanvas 桌面版 0.2.0`）与 `ce9c7d7`（`docs: record the v0.2.0 release and correct the now-stale "unfixed" wording`），本地 **ahead=0 / behind=2** ⇒ `git merge --ff-only origin/main` **快进成功**（无冲突、无本地改动被覆盖）。核验：`git rev-parse HEAD` 与 `git ls-remote origin refs/heads/main` **同为 `ce9c7d7`**，工作区 clean、divergence 0/0；标签 **`v0.2.0` 随 fetch 到了本地**（`v0.1.0` 仍在）。远端带来的是**发布批次**：`tauri.conf.json` 与 `Cargo.toml` 由 0.1.0 提到 **0.2.0**（`Cargo.lock` 随之刷新），以及 README 与本文档的发布记录（含三份产物的字节数与哈希、Release id、`tag=v0.2.0`）。
- **本机独立复跑这套最新代码（2026-09-23，不是转抄）**：`npm test` **215 文件 / 2611 用例通过 + 1 todo**、`npm run typecheck` **6 个 workspace exit 0**、`npm run lint` **0 error / 14 warning**、`npm run test:rust` **232 例通过 + 3 ignored / 0 失败** —— 与协作者记下的读数**逐项一致**。**如实**：这次**没有**在本机重跑 `npm run bundle` 与 Playwright —— 前者由协作者在发布时跑过（exit 0，产物与哈希记在上面那节），后者他们记的是 **140/140**；我这边只做只读复核，不把它写成"我跑过"。
...
>
> **这一行是 2026-09-21 那一刻的快照，此后不再更新**（它自称"唯一权威"已经过期了）。后续实测把它推进到 —— **2026-09-23 收口值**：`npm test` **215 文件 / 2611 用例通过 + 1 todo**（零失败）、`npm run typecheck` **6 个 workspace exit 0**、`npm run lint` **0 error / 14 warning**、`npm run build` **exit 0**、`npm run test:rust` **232 例通过 + 3 ignored / 0 失败**、`cargo clippy --all-targets -- -D warnings` **exit 0**。逐批证据见上文各节（「修复轮 A / B 系列」「密钥用例」「`invalid_type@envelope`」「工作区判据」「密钥」「正四面体」…）。
>
> **本机环境两条（都实测过，写给下一次）**：① 直连 `github.com` 会被重置，git 必须走本机代理 —— 本仓库 `--local` 已配 `http.proxy` / `https.proxy` = `http://127.0.0.1:7890`（全局配置未动）；② **`test` / `typecheck` / `build` 全绿不等于"运行中的窗口在跑新代码"** —— dev 实例的 Vite 转换缓存会卡住（本次实测：`GET localhost:5173/src/agent/modelPlanner.ts` 返回的代码里**只有调用点、没有定义**，而磁盘上两个函数都在），要在真实窗口里复现问题，**先重启 dev 实例**。
