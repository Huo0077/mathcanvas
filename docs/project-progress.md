...
> **当前基线（唯一权威，2026-09-17 在"全身大体检（十批）+ e2e 构建修复 + 平面网格固定"之后实测）**：`npm.cmd test` **107 个测试文件、1194 个用例通过**；4 个 workspace 类型检查通过；ESLint **0 error、14 条 warning**；生产构建通过（Vite 仍提示主 bundle 超过 500 KB）；Playwright **84/84** 通过（**现在真的跑的是当前工作区的构建产物**，见下）。
...
- **明确不修、只记录**（都写进 `docs/feature-catalog.md` 的"体检结论与已知限制"）：
  ...
  - **`circle3` 没有重算分支**：它声明了依赖中心点，但重算里没有对应分支；画布无渲染分支、界面无入口，只有手写 `.mgeo` 才可能遇到。
  - 交点预览把"两者确实不相交"也归为 `insufficient`（状态栏文案本身是对的，例如"两组面之间没有交线"）；改标签要连带改状态栏分支，收益只是措辞，故保留现状。
  - `boolean3d` 的 `O(V²)` 键重建、`DrawingViewport` 的移动阈值会丢弃编辑、每帧自动保存草稿等性能 / 交互项，已记录待办。
