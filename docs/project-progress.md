...
- **切片 1（已完成）**：内核**两个凸实体的布尔交集** `packages/geometry-kernel/src/boolean3d.ts`。
...
- **切片 2/3（未做，明确记录）**：①画布自动枚举场景里**所有**两两交线（照平面画布那样一直可点），并支持多份虚线预览同时存在（现在 `ThreeScenePreview` 是"单份预览"，要扩成列表）；②点击创建**交面图元**（布尔交集实体化成独立的点/棱/面 + 多面体：进对象列表、检查器显示来源与体积/面积、随来源重算、参与删除级联）；③`intersectionLine` 目前**检查器里没有属性块**（只有名字与外观），要补"来源 A 与 B / 段数 / 总长度 / 状态"；④性能：两两布尔交集必须按需计算并缓存（文档变化时算，不能每帧算）。
