...
上一批基线（**2026-09-21，修掉"非空画布上的两处必然失败"之后实测**）：`npm.cmd test` 为 **193 个测试文件、2207 个用例全部通过（零跳过）**；**6 个 workspace**（含 `@draw/desktop`）类型检查通过（exit 0）；ESLint **0 error / 14 warning**（14 条为既有基线）；Playwright Chromium **124/124** 通过（global setup **按当前工作区重新构建** `build-check/mathcanvas-current` 再预览，因此结果对应工作区源码，而不是该目录里上一次构建的产物）。**Rust 侧本批没有改动、也没有复跑**（最近一次实测：`npm run test:rust` **193 例通过 + 1 例 `#[ignore]`**，`cargo clippy --all-targets` **exit 0 / 零警告**），`npm run build` exit 0 并产出 `apps/desktop/src-tauri/target/release/mathcanvas-desktop.exe`（16.0 MB）——**并且真的反复启动过它**：这一批就是靠 CDP 驱动**真窗口**跑完整链路验证的（窗口标题 `MathCanvas`，WebView2 加载成功、既有工作台渲染出来，见 `D:\数学画布\desktop-shell.png`）。

### 桌面外壳、密钥库、provider 转发与仓储（G1，2026-09-21）

**唯一的外部阻塞已解除**：Rust 工具链装好了（`rustup 1.29.1` / `cargo 1.98.1` / `rustc 1.98.1`，`stable-x86_64-pc-windows-msvc`），前置条件 `npx tauri info` 报 Windows 10.0.26100 · WebView2 153.0.4234.48 · MSVC 生成工具 2022（`link.exe` 14.44.35207）。winget 写的是**用户级 PATH**，所以安装前就开着的终端看不到 `cargo` / `npm` —— `scripts/toolchain.mjs` 在**子进程的 PATH** 里补上它们，`npm run test:rust` 与桌面构建都走它。**2026-09-21 在这台机器上重装并复核过**：rustup 缺 `link.exe` 时 `rustc` 能编译但**链接必定失败**（`error: linker 'link.exe' not found`），补装 `Microsoft.VisualStudio.2022.BuildTools`（组件 `Microsoft.VisualStudio.Component.VC.Tools.x86.x64`）之后一次真实链接通过；装完之后**已经开着的进程仍然看不到 `cargo`**（`npx tauri info` 会报 rustc/cargo 未安装），重开终端或走 `scripts/toolchain.mjs` 即可。
...
- **失败也带分类**：`provider_run` / `provider_check` 的 `Err` 值就是 `ModelEvent` 的 `failed` 形状（`kind` / `failure` / `message` / `retryable`），前端**照抄**它而不是从一句话里重新猜分类 —— 从文本里认分类是一次必然会漏的判断。（命令的 `Err` 到前端时是一串 JSON 文本，所以 `readFailureContract` 解析一次，而不是把那坨 JSON 显示给用户。）
- **能力证据探针**（Task 1.4 Step 5，`provider_check`）：四条探针**各发各的请求**（文本、JSON、一张 1×1 的 PNG、一次带工具表的请求），逐条下结论。判据的核心是计划原文那句"**一次成功的文本 ping 不许把 tools 或 vision 打勾**"，以及"**这家不支持**与**我们发错了**必须分开"——图片/工具请求被 400 拒记 `failed`（形状是我们拼的，要改代码），而认证失败、连不上、没有密钥、模型不肯配合一律留 `unknown`（它们对"支不支持"什么都不说明）。它是**用户按下去才发生**的：一次探测花掉四发真请求，按钮上写明了代价，没有密钥时禁用。**一处如实的保留**：探针的请求形状照三家公开文档拼、**没有对真实服务跑过**。
- **附件两阶段写 + `.mcanvas` 打包**（Task 1.6 Step 4/5）：附件落在 `attachments/`，**名字就是内容的 SHA-256**（同一份只存一次、随时能自验）。写入是两阶段的：**先按哈希校验并原子落盘**（`tmp/*.part` → `sync_all` → `rename`），**再写库引用** —— 崩溃落在中间只会留下**孤儿 blob**（GC 收掉），而反过来先写库会留下**悬空引用**（用户看到"附件丢失"）。引用记在**快照**上而不是 head 上（撤销回旧版本时那一版的图必须还在），GC 只删没有任何快照引用的 blob 并带 60 秒宽限期（避开"文件已写、引用还没写"的窗口）。`.mcanvas` 是**自己写的一小块 zip**（只写 stored 条目，用户拿 7-Zip 也能打开看一眼），读取器刻意不是通用 unzip：只认 stored、拒加密、拒 ZIP64、拒任何指向容器外的名字。导入那一侧校验版本、路径安全、**文档哈希**、**附件哈希**、大小上限，以及**清单与条目一一对应（两个方向都查：声明了却没有＝神秘丢失，有却没声明＝夹带）**。包里**永远没有密钥**（只有 `profileId` / `secretRef` 这种引用，用例逐字扫过）。`.mgeo` 格式**一个字节没改**。**一处如实的缺口**：这些命令还没有界面入口，目前只能通过 IPC 调用。
- **模型服务界面是 CC Switch 那种清单**（2026-09-21，用户口径）：左上角一个**加号**添加 API key；保存成功**立刻跑一次能力验证**；验证过的那一份就以**一张卡片**出现在清单里，可以有很多张；每张卡片有名字、模型名、协议与网络策略、密钥状态、健康状态、**四个能力徽章**（工具调用 / 严格 JSON / 图像 / 流式 —— 只有 `verified` 算已验证）、一句说明，以及四个动作：**使用 / 验证 / 编辑 / 删除**。**"使用中"那一栏用左侧竖条 + 写着「使用中」的徽章标出来**（只靠颜色的话，灰度截图与色觉障碍下就分不出来了）。切换**只动一个字段、不碰任何配置的修订号** —— 能力证据挂在修订号上，切换若推高修订号会把四个徽章全清掉。验证没通过时那一栏**仍然出现**（配置是真的存下来了），只是状态如实说"未通过验证"并能就地重试。**「使用中」现在真的驱动规划器**（G2 接线）：Agent 工作区的那一轮运行由它决定走哪个 provider 与哪条通道，`RunContext.textProfileId` 也如实写它。
- **规划器接到真实 provider**（2026-09-21，G2）：`apps/web/src/agent/modelPlanner.ts` 自己现取「使用中」的那一份（有没有桌面外壳 / 有没有选过 / **有没有密钥**），按 `isCapabilityVerified` 选通道（`json` 已验证 → 严格 JSON 通道；否则文本通道并允许一层 ```json 围栏）；动作菜单按阶段写进提示词（`plan.set_plan` 在表里才允许返回计划）；provider 失败**抛**带 `code` + `retryable` 的错，解析失败**原样交出去**让协调器走一次性修复（第二次请求带字段路径，且**不回显**模型上一轮的原话）；用户按停止时真的调 `provider_cancel`。**重试按策略表来**（`RecoveryController.decide`）：`transport` / 429 / 5xx 重试同一请求（上限 3 次，**并从 `PlanRequest.budget` 里扣** `network` + `generation` —— 重试不是免费的），认证 / 权限 / 未归类**一次都不多发**，流损坏**不自己重发**（那需要一份新上下文，而上下文是协调器组装的，所以如实交回给用户的重试）。**一处如实缺口**：`native_tools` 通道到不了 —— `provider_run` 的入参只有 `messages`，IPC 契约里没有放工具表的位置，所以那一条是**如实拒绝**而不是静默降级。
- **项目包界面入口**（2026-09-21，Task 1.6 Step 4/5）：文件命令那一组的「项目包…」打开一个面板，做三件事 —— **导出 `.mcanvas`**（目的地填一次就记住，自动补 `.mcanvas` 扩展名）、**导入 `.mcanvas`**（换一世：新 epoch，于是可能还在途的旧自动保存立刻 CAS 失败）、**附件**（选本地文件 → 前端按**原始字节**算 SHA-256 → 两阶段写；另有孤儿回收）。**三处如实说明摆在界面上**：没有原生文件选择框（能力清单里只有 `core:default`，没有 dialog 插件，所以路径要自己填）、附件列表由两处合成（**这一版快照引用的** —— 从项目库读回来，重开应用之后仍然数得出来 —— 与**本次会话附加的**，后者带本地文件名）、导入是换一世。**不拿假的 generation 去写**：文档还没落过盘时如实拒绝附加。在浏览器里整块功能如实报"需要桌面版"。
...
- 前端在 `apps/web/src/services/`：`desktopRuntime.ts` / `secretClient.ts` / `providerProfileClient.ts` / `documentRepository.ts` / `documentPersistence.ts` / `modelClient.ts` / `projectPackageClient.ts`（`.mcanvas` 与附件的五条命令）—— **在浏览器里跑是正常状态**（不是错误），且**"没有桌面外壳"与"IPC 失败"分开报**（前者正常、后者要给原因）；**缺字段一律当"还没实现"**（危险的那种默认值是"缺字段当可用"）。项目包面板在 `components/ProjectPackagePanel.tsx`（入口在文件命令那一组的「项目包…」）。
...
- **安装包已经真的构建出来了**（2026-09-21，此前记的是"打包安装器属 Task 5.4、未实测"）：`npm --workspace @draw/desktop run bundle` 走 WiX + NSIS，**工具链真的下载并成功产出两个安装包**（`MathCanvas_0.1.0_x64_en-US.msi` 6.3 MB、`MathCanvas_0.1.0_x64-setup.exe` 4.6 MB）与免安装 exe（16.0 MB），三份都上传到 **[GitHub Release `v0.1.0`](https://github.com/Huo0077/mathcanvas/releases/tag/v0.1.0)**（tag 指向 `72e94a0`）。**如实**：这两个安装包**没有在本机实际安装过**（只是构建成功）；免安装 exe 那份是反复启动、并用 CDP 驱动真窗口跑过真模型往返的。
- **未做（如实）**：`tauri dev` 热重载路径未实测；图标仍是脚手架默认；密钥的**手动重启验证**（存 → 重启 → 检测 → 轮换 → 删除）尚未做；`vision` 探针**没有对真实服务跑过**（`deepseek-chat` 是纯文本模型），Anthropic 与 Ollama 两档也没有真实往返；**整条链路（窗口 → 规划器 → 协调器 → 草稿）需要在本机配置一份服务并点一遍**（设置表单在 WebView2 窗口里，无法从外部驱动）。本来在这里的三条已在 2026-09-21 修完：`native_tools` 通道（`provider_run` 现在收 `tools`，并按**存下来的证据**再判一次 `tools_verified`）、**附件列举命令**（`read_document_attachments`）、以及**"探针的请求形状没有对真实服务跑过"**（`tests/provider_live.rs` 真的打了 DeepSeek，并因此抓出一个非流式响应被静默吞掉的解码缺陷）。

**最新一轮（2026-09-21，用户截图报障）：非空画布上的两处必然失败已修**。用户让 Agent 处理一条直四棱柱的题，运行以 `compile_failed: duplicate object id` 结束 —— 查下去发现**只要画布上已经有对象，Agent 就必然失败**，而且是两处、第一处把第二处挡着看不见：

1. **动作层的 id 分配器只会数数**。`createIdAllocator()` 每种 kind 的计数器都从 1 开始，**不知道目标文档里已有哪些 id**：画布上已有 `solid-1`（用户先前建的那个立方体），新动作又被发成 `solid-1`，`validatePatch` 判重拒绝。手工路径的 `nextPrimitiveId`（`App.tsx`）**一直是**"扫已有 id 取下一个空位"，漂移成更弱一份的只有动作层。现在 `createIdAllocator(taken)` 收**占用集**并跳过已占用的空位（允许稀疏占用），草稿存储 / 几何 worker / 手工删除三处调用方都把目标文档的 primitive id 传进去；**别名幂等原样保留**。
2. **规范化哈希与 JSON 语义不一致**（修掉第 1 条后当场暴露）。应用**启动时恢复草稿**会走 `migrateLegacySolids`，物化出来的子对象带着 `style: undefined` / `label: undefined`；文档的**每一处**落盘与比对（`JSON.stringify`、CAS 用的 `contentFingerprint`）都当"没这个字段"，只有 Agent 的预览哈希（`canonicalContentHash`）把 `undefined` 当垃圾抛出去 —— 于是"画布上有一个立体"就等于"运行必然失败"，而用户看到的只有一句内部函数名 `canonicalContentHash: unsupported value of type undefined`。现在哈希与 JSON 对齐（`undefined` = 没有这个字段、数组里的 `undefined` 记作 `null`），真正无法表达的值（函数 / symbol / 非有限数）**照旧拒绝但会报出路径**；迁移里那处**有意**写的 `label: undefined` 改成**删掉这个键**（产生数据的地方也不许写出这种键）。

**实测**（重建 release exe → 重启真窗口 → 真 DeepSeek，画布上已有一个立方体、恢复时已迁移成 28 个图元）：`再建一个棱长 2 的立方体` → 等待你确认（共 29 个）→ 已提交 → 状态栏「对象 **29**」、对象列表出现「立方体(棱长2)」；用户原来那条直四棱柱 → **草稿成型**（共 30 个 · 3 条假设 · **0 个页面错误**），刻意留在确认面板等用户过目。**新增 11 例单测**（分配器 3 / 草稿 2 / worker 1 / 组装运行时 1 / 规范化哈希 3 / 模板迁移 1）+ **2 条 e2e 回归**（先有对象再建同类；**刷新恢复之后再起草**）。

**同日更早一轮（2026-09-21）交付与修复**：①**传输层与动作层的引用形状不一致**（`object.update_inputs` / `dynamic.bind_point` / `dynamic.bind_curve` 三个动作此前**不存在任何一种能同时通过校验并被正确编译的输入**）已修，并由 `packages/agent-core/src/planToCompile.seam.test.ts` 用**已校验的输出**钉住整条接缝；②`draftStore.previewHash` 换成真 SHA-256（`canonicalContentHash`）；③**补上"假设"这一节的数据面**：计划信封新增可选 `assumptions`，经 `onPlanParsed` → 运行时 → 运行器 → 草稿视图 → 确认面板贯通，新增 `AssumptionList.tsx`；④**让"同意不可伪造"从注释变成代码**：`HostBridge` 原先只检查 nonce 未消费 / `runId` / 是否过期 / `previewHash` —— 这四条调用方自己就能凑齐，于是**手搓一份同意也能提交**；现在桥里记着自己铸造过的 nonce（`minted`），没铸造过的一律 `unminted_consent`；⑤**把"工具"从声明接到可执行**：`ToolCallRequest` 原先**没有工具名也没有参数**（所以"接上 ToolPort"是空话），现在补上 `toolId`/`input`，新增 `toolDispatch.ts` 做名字→实现的那一段，`AgentRuntime.callTool` 是宿主入口；⑥**新增 `ToolTracePanel.tsx`**：用户可见的短摘要轨迹 + **默认关着**的开发者详细诊断；⑦**worker 的 diff / check / artifact 信封**（`WorkerSuccess` 必须带齐 `changed` / `diff` / `check` / `artifact`，缺一个就拒绝）；⑧**上下文与工具真正交给规划器**：`PlanRequest` 增加 `model: { context, tools }`，协调器在观察之后、发请求之前组装（且两次尝试看到同一份）；⑨**观察者交出事实文本**（它从第一版起就在算那个数组，只是没放进返回值 —— 是变异检验抓出来的，因为既有用例都是空文档）**+ 技能清单成为可用动作的唯一来源**（调用方说请求哪些技能，运行时去清单取动作，而不是让调用方直接给动作名）。

**Agent 相关的验证重点**：`e2e/agent-flow.spec.ts` 五条 —— ①**一句话 → 草稿预览 → 确认前画布为空 → 确认 → 画布出现对象 → `Ctrl+Z` 一步撤销回空**；②丢弃草稿后文档与历史都不动；③认不出时给"需要补充信息"而不是编一段回答；④**画布上已经有一个同类对象时，第二份草稿照样出现并落到 2 个对象**（分配器占用集的回归）；⑤**刷新页面**（= 重启 + 恢复 + `migrateLegacySolids`）之后再发一次请求，草稿照样成型（规范化哈希那条的回归）。单测侧另有：`agentRunner.test.ts`（暂存阶段不动文档 / 确认后恰好一步历史 / 拒绝二次提交）、`agentRuntime.test.ts`（**一个替身都不用**：真草稿存储 + 真宿主桥 + 真协调器；含"非空文档上的草稿"那条）、`idAllocator.test.ts`（占用集 + 别名幂等）、`pipeline.test.ts`（G0.5 四条 Gate 端到端）。
...
- [**外部审查报告（2026-09-22）**](docs/research/external-audit-2026-09-22/)：四路只读审查的原始报告 —— [几何内核 / DSL](docs/research/external-audit-2026-09-22/geometry-dsl-audit.md)、[scene-graph 与持久化](docs/research/external-audit-2026-09-22/scene-graph-persistence-audit.md)、[Agent 流水线](docs/research/external-audit-2026-09-22/agent-pipeline-audit.md)、[桌面外壳与会话存储](docs/research/external-audit-2026-09-22/desktop-conversation-audit.md)；共 2 Critical / 14 Important / 16 Minor，**均未修复**。
...
- [**端到端验收清单：桌面版 Agent 走一轮真实模型**](docs/acceptance/2026-09-21-end-to-end-agent-checklist.md)：最后一段路只能由人在本机点（设置表单在 WebView2 窗口里）。清单逐步写了"该看到什么"，并附一张**已知原因对照表**（哪些失败是预期的，例如 DeepSeek 的工具徽章会记 `failed`）。
- **实施计划**（`docs/superpowers/plans/`，共 **21** 份，全部带复核状态）：MVP 基础、动态数学平台、公式画布、下一步数学平台、数值鲁棒性、P3+P6、点驱动 3D、3D 测量质量、工程工作台层次化、P7 工程制图、Ribbon UI、3D Auto-Fit、**解析二次曲面 A1**、3D 动点宿主、3D 渲染管道去重建化、**立体几何最后一轮（约束轨道 / 拖动旋转 / 测量数字 / UI 对齐，7 片）**、**[桌面 Agent（G0 / G0.5 / G1 / G2 / P5 / G5，逐任务带复核状态）](docs/superpowers/plans/2026-09-18-desktop-agent-implementation-plan.md)**，以及 2026-09-22 落地的四份——[**Solid/Prism**](docs/superpowers/plans/2026-09-21-solid-prism-implementation-plan.md)、[**Reactive DAG**](docs/superpowers/plans/2026-09-21-reactive-dag-implementation-plan.md)、[**Agent DSL 与参数审计**](docs/superpowers/plans/2026-09-21-agent-dsl-implementation-plan.md)、[**多会话上下文**](docs/superpowers/plans/2026-09-21-conversation-context-implementation-plan.md)（复选框状态即逐条验收记录；其中 Reactive DAG 的「2D 预览几何也走图」一条按裁决**保持未勾选**并注明原因）。
- **设计规格**（`docs/superpowers/specs/`，共 **14** 份，全部带复核状态）：动态绘图平台、数值鲁棒性、点驱动 3D、3D 测量质量、工程工作台、P7 工程制图、Ribbon UI、截面 / 截线图元、3D 视口与内核重构、**解析二次曲面与真圆（A1）**、**交面按支撑曲面分组（A2）**、平面动点系统、**立体几何最后一轮（约束轨道 / 拖动旋转 / 测量数字 / UI 对齐）**、[**增量几何内核与多会话 Agent**](docs/superpowers/specs/2026-09-21-incremental-geometry-agent-and-conversations-design.md)（2026-09-21/22 四份计划的共同真源：统一 Solid、Reactive DAG、Agent 编译六层、多会话上下文与会话事实）。
...
6. **Agent 的下一段路**（P4 已按用户要求开工，不再是排除项）：按 [`桌面 Agent 实施计划`](docs/superpowers/plans/2026-09-18-desktop-agent-implementation-plan.md)——
   - ~~**G1 唯一的硬阻塞是环境问题**~~ **已解除（2026-09-21）**：`winget install Rustlang.Rustup` 装好了工具链（`rustup 1.29.1` / `cargo 1.98.1` / `rustc 1.98.1`），G1 的六个任务已全部动手（1.1 外壳 / 1.2 SecretStore / 1.3 provider 配置 / 1.4 适配器 + 真实转发 + 能力探针 / 1.5 回环代理 / 1.6 SQLite 仓储），进度见 [`docs/project-progress.md`](docs/project-progress.md) 的「G1 阶段状态」表（**唯一真源**）。**G1 的代码已全齐**（含 `.mcanvas` 打包与附件两阶段写 / GC）、模型服务界面已按用户口径重做、**附件与 `.mcanvas` 导出/导入的界面入口已接**（「项目包…」），**「使用中」的那一份也已被规划器消费**（G2 接线）。**真正剩下的**是：**用户本机做的密钥手动重启验证**（Gate ②）、`vision` 探针与 Anthropic 档的**真实往返**（`deepseek-chat` 是纯文本模型，图像那条探针没有对真实服务跑过）、`tauri dev` 热重载未实测（**安装包已经能构建出来并上传了 Release**，只是没在本机安装过）。`native_tools` 通道与附件列举命令**已经补上了**（见进度档案的「G2 补口」与「项目包：附件列举命令补上」两节）。
   - **G2 剩余（不依赖 Rust）**：`ToolPort` 已接到宿主分发表（`AgentRuntime.callTool`）、`AssumptionList.tsx` / `ToolTracePanel.tsx` 已在确认面板与运行状态卡里、worker 的 diff/check/artifact 信封已补 —— 这几条在「最新一轮（2026-09-21）」里已交付。**规划器接真模型已完成**（见上文与进度档案的「G2 接线」）。接下来属 P5（图像输入）与真实模型下的人工复核。
   - **真实运行暴露并修掉的三处缺陷**（都已 RED→GREEN 修完并进回归）：①`draftStore` 每次暂存新建分配器 → 同一草稿的第二次暂存分配的 id 与候选撞车（用户看到的是"暂存成功但内容没变"）；②**非空画布上新建同类对象必然 `duplicate object id`**（分配器不知道文档里已经有哪些 id）；③**恢复过的文档哈希不出来**（`migrateLegacySolids` 物化的子对象带 `undefined`，而规范化哈希当时把 `undefined` 当垃圾，报出一句内部函数名）。后两条的现场取证、RED→GREEN 与真窗口实测见上文「最新一轮（2026-09-21，用户截图报障）」。
   - ~~**一处待办**：`draftStore.previewHash` 存的是规范 JSON 字符串而不是哈希~~ **已修（2026-09-21）**：`draftStore` 改用 `@draw/agent-core` 的 `canonicalContentHash`（真 SHA-256，64 位十六进制），有用例钉住形状、并复验了 Compare-and-Swap 那条链。
