...
- Ribbon 支持按钮或 `Ctrl + F1` 折叠、标签临时呼出、图钉固定；右侧 Inspector 根据选择状态展示属性，并在无选择时保持精简。右侧面板不再显示几何约束、智能体（Agent）与**「精确形式」块**（后者按用户要求于 2026-09-18 删除）；约束数据、校验与 `.mgeo` 编码保持不变，教学测量入口位于默认可见的数据区。
