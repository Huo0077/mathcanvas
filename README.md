...
- **偏移 / 修剪 / 延伸**：偏移按给定距离**新建**平行对象（折线按相邻偏移线求斜接，圆/圆弧改半径）；修剪保留目标 a 端所在的一半，延伸把目标 b 端拉到边界，两者都要求选中 2 个图元且顺序为「先边界、后目标」。几何在 `packages/geometry-kernel/src/editing.ts`，前置条件与补丁组装在 `apps/web/src/draftEditing.ts`。
...
当前验证基线（2026-09-16）：`npm.cmd test` 为 65 个测试文件、672 个用例通过；4 个 workspace 类型检查通过；ESLint 0 个 error、40 条 warning；Web 生产构建通过；Playwright Chromium 43/43 通过，覆盖 Ribbon、跨工作区操作、CAD 图纸填充、CAD 2D 绘图交互（固定坐标窗口 / 橡皮筋预览 / 端点与切点捕捉 / 栅格捕捉 / 夹点编辑 / 方向框选 / 坐标键入 / 线宽与命中带 / 偏移与修剪延伸）与 1440px/768px/390px 视口。Vite 仍提示主 bundle 超过 500 KB（当前约 1.51 MB，gzip 约 476 KB）；Vitest 的 jsdom 3D 测试仍会输出 WebGL context 未实现提示。首次运行需先执行 `npx playwright install chromium`，否则会报缺少浏览器可执行文件。
...
Ribbon 基线、2026-09-16 后续 UI 优化（Task 7-13）与工程制图视觉重做（Task 14）均已完成并通过当前自动化验证。下一批候选方向：CAD 视口边界手动拖动（当前只有自动尺寸与固定矩形）、剖切平面可调（法向量与偏移量）、等长/等角约束（需要先定角度表示）、B-rep/DWG 导入与自动尺寸布局；这些尚未编码。P4 Agent 与 P5 题图解析仍在排除范围内。
