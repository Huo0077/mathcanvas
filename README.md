...
- **交线与交面是自动铺开的独立图元**（2026-09-17）：画布自动枚举场景里**所有两两相交的顶层实体**，每一对给两份可独立点击的预览——虚线交线（两个表面的公共边界）与半透明交面片（两个实体公共区域的整体表面，即**布尔交集**）；点虚线创建 `intersectionLine`，点半透明面片创建 `intersectionSolid` 交面图元。两者都进对象列表、随来源重算、删来源时级联注销，检查器给出「来源 / 段数与总长度」与「体积 / 表面积 / 面数 / 顶点数 / 状态」。性能上按来源几何签名缓存每一对的结果（只动一个实体时其余对直接沿用），包围盒先筛掉不相交的对，单次扫描最多算 12 处布尔交集（交线不受限）。布尔交集只对凸实体成立，非凸输入给诊断而不是近似。
- **截面 / 截线也可以"点"出来**：选中含面环的对象时画布立即出现**虚线预览**——只选一个实体时预览默认剖切平面的截面。悬停高亮并提示「点击创建为图元」，**点击即创建**持久化的截面 / 截线图元（进文档、可撤销、随来源自动重算）；顶点手柄优先于预览，棱不优先（预览的命中区就是画布上真画出来的那份几何）。求交在内核 `intersections3d.ts`（两个面环各自被对方平面裁剪成区间后求区间交，共面直接短路，重复线段去重），布尔交集在内核 `boolean3d.ts`（凸实体半空间依次裁剪，切口面按平面内角度排序）。
