...
- **未做（如实）**：`tauri dev` 热重载路径未实测；打包安装器（`tauri build` 带 bundle）属 Task 5.4；图标仍是脚手架默认；密钥的**手动重启验证**（存 → 重启 → 检测 → 轮换 → 删除）尚未做；探针的请求形状**没有对真实服务跑过**（照公开文档拼）；**`native_tools` 通道到不了**（`provider_run` 契约里没有放工具表的位置，需要 Rust 侧加一个 `tools` 参数）；**附件列举命令缺失**（所以界面上的附件列表只覆盖本次会话）；**真实模型的一轮往返没有跑过**（本机没有配过任何 provider、也没有密钥），前端与 IPC 之间用假的 `__TAURI_INTERNALS__` 覆盖了各条路径。
