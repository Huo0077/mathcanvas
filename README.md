...
- 顶部采用约 40px 系统栏、32px 工作区标签栏和约 80px Word 风格 Ribbon；平面（当前内部名 `conics`）、立体几何与工程制图共享命令分组，避免重复的旧工具栏。
- Ribbon 支持按钮或 `Ctrl + F1` 折叠、标签临时呼出、图钉固定；右侧 Inspector 根据选择状态展示属性，并在无选择时保持精简。
...
- 3D 新建点自动提供 A、B、C 等课堂常用标签；2D 新建点当前使用「新点 A」「新点 B」等标签。选中图元后可在属性栏编辑名称，平面工作区统一使用 A/B/C 的计划改动尚未实现。
- 鼠标悬停图元时显示当前接触点的画布坐标。
...
- 右侧上下文 Inspector 当前分为数据、外观、约束、工程标注四个页签；无选择时显示当前图纸、视图、图层、单位和命令提示，并列出工程标注/图纸视图引用到的来源对象，已删除的来源标记为「来源已删除」。后续计划将移除约束和智能体展示，但保留文档数据与真实测量能力。
