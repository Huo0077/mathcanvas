...
- **截面 / 截线可以直接"点"出来**：选中含面环的对象时画布立即出现**虚线预览**，不需要先点命令——只选一个实体时预览默认剖切平面的截面，选中两个对象时预览两者面环之间的**真实交线**（`面交线 · N 段`）。悬停高亮并提示「点击创建为图元」，**点击即创建**持久化的截面 / 截线图元（进文档、可撤销、随来源自动重算）；创建优先级低于点/棱拾取，因此不会抢走顶点手柄。求交在内核 `intersections3d.ts`（两个面环各自被对方平面裁剪成区间后求区间交，共面直接短路，重复线段去重）。
