...
- **动点（平面几何，2026-09-17 重做）**：任何点都可以绑定到一条曲线（直线 / 线段 / 射线 / 折线 / 圆 / 圆弧 / 函数图像 / 椭圆 / 抛物线 / 双曲线）成为动点，绑定参数是该曲线的**自然参数**（直线是仿射比例、圆与椭圆是角度、函数图像是 x 本身、抛物线与双曲线是轴向参数），所以「能滑多远」由曲线自己决定：直线无限、射线只向前、线段与折线有界、圆与椭圆整周。三种驱动方式写同一个参数、完全等价：画布上直接**拖动**、属性栏**「路径参数」输入框**、「动效演示」的**滑块与播放**（循环 / 单次 / 往返）；抛物线与双曲线另有可编辑的「参数域」，双曲线可选择「分支」。
