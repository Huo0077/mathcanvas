...
- **视觉规范（2026-09-18 极简主义重构 + 深色 chrome）**：冷白 `#F8FAFC` 背景、`#E2E8F0` 极浅分割、深板岩蓝 `#2C3E50` 文字图标，强调色为板岩蓝 `#3D5A80`；**顶栏与左侧模块栏同为深色 chrome**（共用 `--color-chrome-*` 令牌，slate-800 → slate-900，与界面其余部分的板岩蓝同色相）形成一道暗色边框，品牌「MathCanvas」在深色底上带浅青方块光标与二十粒亮青浮动粒子；面板之间**去掉生硬边框、改用柔和弥散投影**分层（微拟物化）；顶栏品牌居中、工具图标为间距宽敞的单色极细线性图标。画布为冷白纸 + 极浅极细网格，几何图形用克制色彩（**直线深红褐、圆板岩蓝**），画布内数学符号（P1 / l1 / c1）走数学衬线字体，界面走无衬线。
