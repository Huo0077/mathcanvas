...
- **参数是唯一真值、坐标是派生缓存**，因此动点永远精确落在约束上，不会在连续拖动后漂离曲线；几何对象构成有向无环图，重算按**拓扑序**一趟完成，测量只在其来源真的变化时重算。
