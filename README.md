...
当前验证基线（**2026-09-21，G2 接线 + G1 第十四批之后实测，含 Rust 侧复跑**）：`npm.cmd test` 为 **191 个测试文件、2182 个用例全部通过（零跳过）**；**6 个 workspace**（含 `@draw/desktop`）类型检查通过（exit 0）；ESLint **0 error / 14 warning**（14 条为既有基线）；Playwright Chromium **122/122** 通过（global setup **按当前工作区重新构建** `build-check/mathcanvas-current` 再预览，因此结果对应工作区源码，而不是该目录里上一次构建的产物）。**Rust 侧本轮真的复跑了**（工具链装好之后）：`npm run test:rust` **179 例通过 + 1 例 `#[ignore]`**（单元 7 + project_repository 19 + provider_adapter 16 + provider_capability 18 + provider_profiles 22 + providers 19 + proxy 16 + proxy_server 16 + repository_package 29 + secrets 8 + shell_smoke 9），`cargo clippy --all-targets` **exit 0 / 零警告**，`npm run build` exit 0 并产出 `apps/desktop/src-tauri/target/release/mathcanvas-desktop.exe`（15.9 MB）——**并且真的启动了它**：窗口标题 `MathCanvas`、12 秒与 25 秒后仍在运行（＝窗口起来、WebView2 加载成功、既有工作台渲染出来了，见 `D:\数学画布\desktop-shell.png`），随后清理无残留。
