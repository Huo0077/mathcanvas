...
- **测量数字常驻画布**（2026-09-17）：立体几何与平面几何都**不需要选中任何对象**，有效测量的数值就直接画在图上（长度 / 距离 / 角度 / 面积 / 体积 / 二面角），随几何实时更新；退化 / 数据不足 / 来源点缺失**不画假数字**。立体几何的辅助线与二面角标记仍按选中显示（那是引导线），选中的那条标签加高亮；平面几何的角度标签落在角平分线上、距离落在垂足与点的中点。画布读数 `data-measurement-labels`。
...
下一批候选方向：
1. **两个画布的测量单位还不统一**（已知不一致，如实留着）：平面角的存量值是**弧度**（单位 `rad`），立体几何的角是**度**（单位 `°`）。两边各自都做到"画布上的数字与属性栏一致"，但彼此没统一；改成度要动内核的读数契约（`evaluatePlanarMeasurement` 的 `value`/`unit`）与导出，属于独立一轮。
2. **3D 顶部两排控件在窄画布下会重叠**（本轮实测到的既有缺陷）：宽度不足时「显示控制」那一排的最后一个按钮（**自动取景**）被「视角控制」那一排的第一个按钮（**自由拖动**）盖住，用户点不到（748px 宽的截图里它确实看不见）。修法：窄宽度下换行、或把次要开关收进「更多」菜单。
